...
class Scoreboard:
    ...
    def __init__(self, api_url):
        ...
        print(f"[SCOREBOARD DEBUG] Initialized with API URL: {api_url}")
        
    def get_scores(self):
        """Get top 10 scores from the API"""
        try:
            print(f"[SCOREBOARD DEBUG] get_scores() called")
            ...
            if (self.cached_scores and self.cache_time and 
                ...
                print(f"[SCOREBOARD DEBUG] Returning cached scores ({len(self.cached_scores)} scores)")
                return self.cached_scores
            
            print(f"[SCOREBOARD DEBUG] Cache expired or empty, fetching fresh scores...")
            ...
            if data.get('success'):
                ...
                print(f"[SCOREBOARD DEBUG] Successfully fetched {len(self.cached_scores)} scores")
                return self.cached_scores
            else:
                print(f"[SCOREBOARD DEBUG] API Error: {data.get('error', 'Unknown error')}")
                return self.cached_scores  # Return cached scores if available
                
        except requests.exceptions.RequestException as e:
            print(f"[SCOREBOARD DEBUG] Network error: {e}")
            return self.cached_scores  # Return cached scores if available
        except Exception as e:
            print(f"[SCOREBOARD DEBUG] Unexpected error: {e}")
            return []
...
class Game:
    def __init__(self):
        ...
        self.scoreboard = Scoreboard('https://script.google.com/macros/s/1wDHRWGpje67CGVdZWNo_cORpVgAwc1Cki-RNElMnLqs/exec')
    ...
    def add_score(self, points, event_type=""):
        ...
        # Log detailed scoring event with new format including game level
        import datetime
        current_time = datetime.datetime.now().strftime("%H:%M:%S.%f")[:-3]  # Include milliseconds
        
        if event_type:
            print(f"[{current_time}] Level {self.level} - {event_type}: +{points}, +{multiplier_gained:.3f}, x{multiplier:.3f}, ={self.score}")
        else:
            print(f"[{current_time}] Level {self.level} - SCORE EVENT: +{points} points, x{multiplier:.2f} multiplier = {multiplied_points} total, Score: {old_score} -> {self.score}, Current Multiplier: {multiplier:.2f}")
        ...
        if pygame.K_c in self.keys_pressed:
            ...
            if hasattr(self, 'scoreboard'):
                ...
                if self.show_scoreboard:
                    self.scoreboard_scores = self.scoreboard.get_scores()
    ...
    def draw(self, dt=0.016):
        ...
        if self.game_state == "waiting":
            ...
            # Draw C-S-L controls text at bottom
            scoreboard_font = pygame.font.Font(None, 24)
            scoreboard_text = scoreboard_font.render("C - S - L", True, (150, 150, 150))
            ...
            if self.show_scoreboard:
                ...
                inst_text = inst_font.render("C = Refresh  |  S = Submit Score  |  L = Close", True, (200, 200, 200))
                    ...
                    if event.type == pygame.QUIT:
                        ...
                        if event.key == pygame.K_SPACE and self.game_state == "waiting":
                            ...
                            if event.key == pygame.K_l:
                                ...
                                if self.show_scoreboard:
                                    self.scoreboard_scores = self.scoreboard.get_scores()
                                    print(f"[SCOREBOARD DEBUG] L key pressed on title screen - toggled scoreboard")
                            ...
                            if event.key == pygame.K_l:
                                ...
                                if self.score > 1000:  # Minimum score threshold
                                    ...
                                    self.player_name_input = ""
                            elif event.key == pygame.K_ESCAPE and self.name_input_active:
                                # Cancel name input
                                self.name_input_active = False
                                self.player_name_input = ""
    ...
    def draw_scoreboard(self, surface):
        ...
        y_offset = 150
        for i, score_line in enumerate(self.scoreboard.format_scores_display(self.scoreboard_scores)):
        ...
        inst_text = font_small.render("Press L to close scoreboard", True, WHITE)
    ...
    def draw_name_input(self, surface):
        ...
        name_text = font.render(self.player_name_input + "_", True, WHITE)
