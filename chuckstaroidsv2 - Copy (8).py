...
class Ship(GameObject):
    def __init__(self, x, y):
        ...
        self.rof_peak_time = 1.0         # Time to reach peak speed
    ...
    def update(self, dt, screen_width=None, screen_height=None, time_dilation_factor=1.0, raw_dt=None, multiplier=1.0):
    ...
    def draw(self, screen):
        ...
        if self.image:
            ...
            # Draw ship shadow first (behind the ship)
            shadow_ship = pygame.transform.scale_by(rotated_ship, 1.1)  # Make shadow 10% bigger
            shadow_ship.fill((0, 0, 0, 255), special_flags=pygame.BLEND_MULT)  # Make it black first
            shadow_ship.set_alpha(107)  # 42% opacity
            ...
            if self.invulnerable and int(self.invulnerable_time * 40) % 2:
                # Flash effect when invulnerable - create a bright cyan tinted version
            ...
            # Draw shadow triangle first (behind the ship)
            shadow_points = []
            for i in range(3):
                angle = self.angle + i * (2 * math.pi / 3)
                x = self.position.x + math.cos(angle) * (self.radius * 1.1) + 10  # 10% bigger radius
                y = self.position.y + math.sin(angle) * (self.radius * 1.1) + 10  # 10% bigger radius
                shadow_points.append((x, y))
            # Create shadow surface for proper alpha blending
            shadow_surface = pygame.Surface((screen.get_width(), screen.get_height()), pygame.SRCALPHA)
            pygame.draw.polygon(shadow_surface, (0, 0, 0, 107), shadow_points)  # 42% opacity
            screen.blit(shadow_surface, (0, 0))
            
            # Draw ship
            color = WHITE
            if self.invulnerable and int(self.invulnerable_time * 40) % 2:
                color = (0, 255, 255)  # Flash bright cyan when invulnerable
            elif self.red_flash_timer > 0:
                color = (255, 0, 0)  # Flash red when taking damage
    ...
    def update_rate_of_fire(self, dt, is_shooting):
        ...
        if is_shooting:
            ...
            if self.rof_progression_time <= self.rof_peak_time:
                # Aggressive ramp up phase: 0.09s to 0.042s over 1 second
...
class Asteroid(GameObject):
    def __init__(self, x, y, size=3):
        ...
        # All asteroids have shadows (hardcoded)
    ...
    def draw_shadow_only(self, screen, screen_width=None, screen_height=None):
        ...
        for pos_x, pos_y in positions:
            if self.image:
                # Draw shadow first (behind the asteroid)
                rotated_asteroid = pygame.transform.rotate(self.image, -math.degrees(self.rotation_angle))
                # Hardcoded shadow properties: 1.2x size, 10px offset, 80% opacity
                shadow_scale = 1.2
                shadow_offset = 10
                shadow_alpha = int(255 * 0.8)  # 80% opacity
                shadow_asteroid = pygame.transform.scale_by(rotated_asteroid, shadow_scale)
                shadow_asteroid.fill((0, 0, 0, 255), special_flags=pygame.BLEND_MULT)  # Make it black first
                shadow_asteroid.set_alpha(shadow_alpha)
                ...
                # Draw shadow polygon first (behind the asteroid)
                # Hardcoded shadow properties: 1.2x size, 10px offset, 80% opacity
                shadow_scale = 1.2
                shadow_offset = 10
                shadow_alpha = int(255 * 0.8)  # 80% opacity
                ...
                for point in self.points:
                    x, y = point
                    # Rotate and scale up shadow dynamically
                    cos_a = math.cos(self.rotation_angle)
                    sin_a = math.sin(self.rotation_angle)
                    rx = (x * shadow_scale) * cos_a - (y * shadow_scale) * sin_a
                    ry = (x * shadow_scale) * sin_a + (y * shadow_scale) * cos_a
                    # Translate to wrapped position with dynamic shadow offset
                    shadow_points.append((rx + pos_x + shadow_offset, ry + pos_y + shadow_offset))
                # Create shadow surface for proper alpha blending
                shadow_surface = pygame.Surface((screen.get_width(), screen.get_height()), pygame.SRCALPHA)
                pygame.draw.polygon(shadow_surface, (0, 0, 0, shadow_alpha), shadow_points, 2)
                screen.blit(shadow_surface, (0, 0))
    ...
    def draw(self, screen, screen_width=None, screen_height=None):
        ...
        for pos_x, pos_y in positions:
            if self.image:
                ...
                # Draw shadow polygon first (behind the asteroid) - all asteroids have shadows
                if self.has_shadow:
                    # Hardcoded shadow properties: 1.2x size, 10px offset, 80% opacity
                    shadow_scale = 1.2
                    shadow_offset = 10
...
class AdvancedUFO(GameObject):
    ...
    def draw_ufo_shadow(self, screen, shake_x=0, shake_y=0):
        ...
        if self.image:
            ...
            rotated_ufo = pygame.transform.rotate(self.image, -math.degrees(rotation_angle) - 90)
            
            shadow_ufo = pygame.transform.scale_by(rotated_ufo, 1.2)  # Make shadow 20% bigger
            shadow_ufo.fill((0, 0, 0, 255), special_flags=pygame.BLEND_MULT)  # Make it black first
            shadow_ufo.set_alpha(shadow_alpha)
            ...
            pygame.draw.ellipse(shadow_surface, (0, 0, 0, shadow_alpha), 
                              (self.position.x - self.radius * 1.2 + 8 + shake_x, self.position.y - self.radius/2 * 1.2 + 8 + shake_y,
                               self.radius * 2 * 1.2, self.radius * 1.2))
            pygame.draw.rect(shadow_surface, (0, 0, 0, shadow_alpha),
                            (self.position.x - self.radius/2 * 1.2 + 8 + shake_x, self.position.y - self.radius/4 * 1.2 + 8 + shake_y,
    ...
    def draw_ufo_smoke(self, screen, shake_x=0, shake_y=0):
        ...
        if thrust_width > 0:  # Only draw if there's thrust
            ...
            try:
                ...
                rotated_smoke = pygame.transform.rotate(smoke_image, -math.degrees(self.angle) - 90 + 90 + 180)
                ...
                # Fallback to triangle smoke - scale based on thrust width (2x wider)
                smoke_points = []
                smoke_radius = max(10, thrust_width // 2)  # Scale triangle size with thrust (2x wider)
                for i in range(3):
                    angle = self.angle + math.pi + i * (2 * math.pi / 3)
                    x = self.position.x + math.cos(angle) * smoke_radius + shake_x
                    y = self.position.y + math.sin(angle) * smoke_radius + shake_y
                    smoke_points.append((x, y))
                
                # Apply 50% transparency to triangle smoke
                smoke_surface = pygame.Surface((smoke_radius * 2, smoke_radius * 2), pygame.SRCALPHA)
                # Adjust points to be relative to the surface center
                adjusted_points = [(p[0] - (self.position.x + shake_x) + smoke_radius, 
                                 p[1] - (self.position.y + shake_y) + smoke_radius) for p in smoke_points]
                pygame.draw.polygon(smoke_surface, (200, 200, 200, 128), adjusted_points)  # Gray smoke with 50% alpha
                screen.blit(smoke_surface, (int(self.position.x + shake_x - smoke_radius), 
                                          int(self.position.y + shake_y - smoke_radius)))
    ...
    def apply_accuracy_modifier(self, base_angle, accuracy_modifier, level_penalty=1.0):
        ...
        # Combine all accuracy modifiers
...
class BossEnemy(GameObject):
    def __init__(self, x, y, direction="right", screen_width=1000, screen_height=750):
        super().__init__(x, y)
        self.active = True
        ...
        # Movement parameters
        self.speed = random.uniform(50, 75)  # 50-75 units per second
        self.amplitude = 20  # 20px amplitude for sine wave
        self.frequency = 0.1  # 0.1 cycle per second
        self.sine_timer = 0.0
        
        # Calculate center line for sine wave (top 2/5 or bottom 4/5)
        ...
        if direction == "right":
            ...
            self.flip_horizontal = True  # Flip when coming from right
        else:  # left
            self.position.x = screen_width + 300  # Start off-screen right
            self.velocity = Vector2D(-self.speed, 0)
            self.flip_horizontal = False  # Don't flip when coming from left
        ...
        try:
            ...
            get_logger().error(f"Failed to load stard.gif: {e}")
            self.image = None
        
        # Set radius for collision detection (if needed later) - 250px radius for 500px image
        self.radius = 250
        
        # Static particle ring for positioning
    ...
    def update(self, dt, screen_width=None, screen_height=None):
        ...
        # Generate static particles if not already generated
        if not self.particles_generated:
            self.generate_static_particles()
        
        # Update static particles to follow boss
        self.update_static_particles()
        
        # Check if boss has crossed the entire screen
        if self.direction == "right" and self.position.x > self.screen_width + 300:
            # Boss has moved completely off the right side
            self.active = False
        elif self.direction == "left" and self.position.x < -300:
            # Boss has moved completely off the left side
            self.active = False
    
    def generate_static_particles(self):
        """Generate a static ring of 12 bright green particles for positioning"""
        if self.particles_generated or not self.active:
            return
            
        # Clear any existing particles
        self.static_particles.clear()
        
        # Generate 12 particles in a perfect circle around the boss center
        for i in range(12):
            # Calculate angle for this particle (evenly distributed around circle)
            angle = (i / 12.0) * 2 * math.pi
            
            # Fixed position on 50px radius circle (100px diameter), offset 15px to the right
            particle_x = self.position.x + 15 + math.cos(angle) * 50
            particle_y = self.position.y + math.sin(angle) * 50
            
            # Bright green color
            particle_color = (0, 255, 0)
            
            # Create static particle (no velocity, long lifetime)
            particle = Particle(particle_x, particle_y, 0, 0, particle_color, 999.0, 3.0)
            self.static_particles.append(particle)
        
        # Generate 12 particles in a 500px line through the center of the circle, rotated 5 degrees counter-clockwise
        ...
        for i in range(12):
            ...
            particle_x = line_center_x + rotated_x
            particle_y = line_center_y + rotated_y
    ...
    def update_static_particles(self):
        """Update static particles to follow boss position"""
        if not self.particles_generated or not self.active:
            return
            
        # Update circle particles (first 12)
        for i in range(12):
            if i < len(self.static_particles):
                angle = (i / 12.0) * 2 * math.pi
                self.static_particles[i].x = self.position.x + 15 + math.cos(angle) * 50
                self.static_particles[i].y = self.position.y + math.sin(angle) * 50
        
        # Update line particles (next 12) - 500px line rotated 5 degrees counter-clockwise
        rotation_angle = -5 * math.pi / 180  # Convert 5 degrees to radians (negative for counter-clockwise)
        cos_angle = math.cos(rotation_angle)
        sin_angle = math.sin(rotation_angle)
        ...
        for i in range(12, 24):
            if i < len(self.static_particles):
                ...
                # Apply rotation and center offset
                self.static_particles[i].x = line_center_x + rotated_x
                self.static_particles[i].y = line_center_y + rotated_y
    
    def draw_static_particles(self, screen):
        """Draw the static particle ring"""
    ...
    def draw(self, screen, screen_width=None, screen_height=None):
        ...
        self.draw_static_particles(screen)
...
class ExplosionSystem:
    ...
    def add_ability_particles(self, x, y, player_angle, player_speed):
        ...
        for i in range(min(num_particles, len(grid_positions))):
            ...
            particle = Particle(particle_x, particle_y, vx, vy, particle_color, 3.0, 1.0)
            self.particles.append(particle)
...
class Game:
    ...
    def trigger_score_pulse(self):
        ...
        # Cap at 5 seconds for 5000+ points
        pulse_duration = min(0.1 + (self.score_change / 100.0) * 0.1, 5.0)
    ...
    def draw_score_with_pulse(self, surface, dt):
        ...
        if self.score_pulse_timer < self.score_pulse_duration:
            ...
            # Calculate size scaling (1x at 1000 points to 3x at 10000+ points)
            if self.score_change >= 1000:
                # Linear scaling from 1x at 1000 points to 3x at 10000+ points
                # Formula: 1.0 + ((points - 1000) / 9000) * 2.0
                size_scale = 1.0 + ((self.score_change - 1000) / 9000.0) * 2.0  # 1.0 to 3.0
    ...
    def spawn_asteroids(self, count=None):
        ...
        asteroid = Asteroid(x, y, random_large)
        # All asteroids have shadows (hardcoded in Asteroid.__init__)
        ...
        x, y = self.get_edge_position()
        asteroid = Asteroid(x, y, random_medium)
        # All asteroids have shadows (hardcoded in Asteroid.__init__)
        self.asteroids.append(asteroid)
        ...
        if self.level >= 2:
            ...
            for _ in range(additional_count):
                ...
                x, y = self.get_edge_position()
                asteroid = Asteroid(x, y, random_size)
                # All asteroids have shadows (hardcoded in Asteroid.__init__)
                self.asteroids.append(asteroid)
    ...
    def handle_input(self, dt):
        # R key restart only works when player is alive (playing state with lives > 0)
        if pygame.K_r in self.keys_pressed:
            if self.game_state == "playing" and self.lives > 0:
                self.restart_game()
                return
            # R key does nothing in other states (waiting, paused, game_over, etc.)
            # Let event-based handling take care of game_over state
            ...
            if asteroid.active:
                if asteroid.size > 1:  # Can still break down
                    ...
                    self.explosions.add_explosion(asteroid.position.x, asteroid.position.y, 
                                                ...
                                                color=(75, 75, 75), asteroid_size=asteroid.size, is_ufo=False)  # Gray
                    # 20% dark brown particles
                    self.explosions.add_explosion(asteroid.position.x, asteroid.position.y, 
                                                num_particles=int(total_particles * 0.20), 
                                                color=(34, 9, 1), asteroid_size=asteroid.size, is_ufo=False)  # Dark brown
                    # 15% red-brown particles
                    self.explosions.add_explosion(asteroid.position.x, asteroid.position.y, 
                                                num_particles=int(total_particles * 0.15), 
                                                color=(98, 23, 8), asteroid_size=asteroid.size, is_ufo=False)  # Red-brown
                    # 10% orange-red particles
                    self.explosions.add_explosion(asteroid.position.x, asteroid.position.y, 
                                                num_particles=int(total_particles * 0.10), 
                                                color=(148, 27, 12), asteroid_size=asteroid.size, is_ufo=False)  # Orange-red
                    # 8% orange particles
                    self.explosions.add_explosion(asteroid.position.x, asteroid.position.y, 
                                                num_particles=int(total_particles * 0.08), 
                                                color=(188, 57, 8), asteroid_size=asteroid.size, is_ufo=False)  # Orange
                    # 7% golden particles
                    self.explosions.add_explosion(asteroid.position.x, asteroid.position.y, 
                                                num_particles=int(total_particles * 0.07), 
                                                color=(246, 170, 28), asteroid_size=asteroid.size, is_ufo=False)  # Golden
    ...
    def generate_title_ability_particles(self):
        ...
        for ufo in self.ufos[:]:
            if ufo.active:
                # Add explosion particles for each destroyed UFO
                total_particles = int(40 * 1.5 * 1.5)  # Same as in-game UFO destruction
                ...
                self.explosions.add_explosion(ufo.position.x, ufo.position.y, 
                                            ...
                                            color=(0, 100, 255), is_ufo=True)
                
                # 10% bright white particles
                self.explosions.add_explosion(ufo.position.x, ufo.position.y, 
                                            num_particles=int(total_particles * 0.10), 
                                            color=(255, 255, 255), is_ufo=True)
            ...
            for ufo in self.ufos[:]:
                ...
                if self.check_wrapped_collision(bullet.position, ufo.get_hitbox_center(), bullet.radius, ufo.radius):
                    ...
                    if ufo.spinout_active:
                        ...
                        if hasattr(ufo, 'spinout_collision_delay_timer') and hasattr(ufo, 'spinout_collision_delay'):
                            if ufo.spinout_collision_delay_timer >= ufo.spinout_collision_delay:
                                ...
                                # Add explosion particles for UFO
                                self.explosions.add_explosion(ufo.position.x, ufo.position.y, 
                                                            num_particles=45, 
                                                            color=(0, 150, 255), is_ufo=True)
                                self.explosions.add_explosion(ufo.position.x, ufo.position.y, 
                                                            num_particles=30, 
                                                            color=(255, 255, 255), is_ufo=True)
                    ...
                    if random.random() < spinout_chance:
                        ...
                        # Add shot hit particles (UFO is still alive during spinout)
                        self.explosions.add_shot_hit_particles(ufo.position.x, ufo.position.y)
            ...
            for asteroid in self.asteroids[:]:
                ...
                if self.check_wrapped_collision(ufo.get_hitbox_center(), asteroid.get_hitbox_center(), ufo.radius, asteroid.radius):
                    ...
                    if not ufo.spinout_active and random.random() < 0.05:  # 5% chance
                        ...
                        # Add hit particles (UFO is still alive during spinout)
                        self.explosions.add_shot_hit_particles(ufo.position.x, ufo.position.y)
            ...
            for j, ufo2 in enumerate(self.ufos[i+1:], i+1):  # Avoid checking same pair twice
                ...
                if self.check_wrapped_collision(ufo1.position, ufo2.position, ufo1.radius, ufo2.radius):
                    ...
                    if ufo1.spinout_active:
                        ...
                        ufo2.active = False
                        self.explosions.add_explosion(ufo2.position.x, ufo2.position.y, 
                                                    num_particles=45, 
                                                    color=(0, 150, 255), is_ufo=True)
                        self.explosions.add_explosion(ufo2.position.x, ufo2.position.y, 
                                                    num_particles=5, 
                        ...
                        try:
                            if hasattr(ufo1, 'spinout_collision_delay_timer') and hasattr(ufo1, 'spinout_collision_delay'):
                                if ufo1.spinout_collision_delay_timer >= ufo1.spinout_collision_delay:
                                    ufo1.active = False
                                    self.explosions.add_explosion(ufo1.position.x, ufo1.position.y, 
                                                                num_particles=45, 
                                                                color=(0, 150, 255), is_ufo=True)
                                    self.explosions.add_explosion(ufo1.position.x, ufo1.position.y, 
                                                                num_particles=5, 
    ...
    def update(self, dt):
        ...
        if self.game_state == "waiting":
            ...
            for boss in self.bosses[:]:
                boss.update(dt, self.current_width, self.current_height)
                if not boss.active:
                    self.bosses.remove(boss)
            
            # Spawn title screen boss immediately if none exist
        ...
        if self.ship:
            ...
            current_multiplier = self.calculate_multiplier()
            self.ship.update(dilated_dt, self.current_width, self.current_height, 1.0, dt, current_multiplier)
        ...
        for boss in self.bosses[:]:
            boss.update(dilated_dt, self.current_width, self.current_height)
            if not boss.active:
                self.bosses.remove(boss)
        
        # Spawn boss on every 3rd level (3, 6, 9, 12...) after delay
        if (self.level % 3 == 0 and not self.boss_spawned_this_level and 
            ...
            # Check for immediate collision with asteroids when boss spawns
            self.check_boss_spawn_collisions(boss)
        
        # Update boss spawn timer
        if self.level % 3 == 0 and not self.boss_spawned_this_level:
    ...
    def advance_level(self):
        ...
        # Check if this is a boss level (every 3rd level: 3, 6, 9, 12...)
        if self.level % 3 == 0:
    ...
    def draw_ship_as_asteroid(self, surface, ship):
        ...
        if ship.velocity.magnitude() > 5:
            ...
            temp_asteroid = Asteroid(ship.position.x, ship.position.y, size=2)
    ...
    def update_title_ship(self, dt):
        ...
        # Update ship physics
        self.title_ship.update(dt, self.current_width, self.current_height, 1.0, dt, 1.0)
    ...
    def draw(self, dt=0.016):
        ...
        if self.game_state == "playing":
            ...
            for boss in self.bosses:
                ...
                # Draw static particles
                boss.draw_static_particles(draw_surface)
                boss.position.x = original_x
                boss.position.y = original_y
            ...
            shadows_with_size3 = [asteroid for asteroid in size_3_asteroids if asteroid.has_shadow]
            ...
            shadows_with_size2 = [asteroid for asteroid in size_2_asteroids if asteroid.has_shadow]
            ...
            shadows_with_size1 = [asteroid for asteroid in size_1_asteroids if asteroid.has_shadow]
            ...
            # Player thrust
            if self.ship and self.ship.thrusting:
                original_x = self.ship.position.x
                original_y = self.ship.position.y
                self.ship.position.x += shake_x
                self.ship.position.y += shake_y
                # Draw thrust flame only
                player_speed = self.ship.velocity.magnitude()
                player_speed_percent = min(player_speed / 1000.0 * 100, 100)
                thrust_width = int((player_speed_percent / 100.0) * 60)
                if thrust_width > 0:
                    thrust_angle = self.ship.angle + math.pi
                    thrust_x = self.ship.position.x + math.cos(thrust_angle) * 30
                    thrust_y = self.ship.position.y + math.sin(thrust_angle) * 30
                    try:
                        thrust_image = pygame.image.load(get_resource_path("fire.gif"))
                        thrust_height = max(10, thrust_width)
                        thrust_image = pygame.transform.scale(thrust_image, (thrust_width, thrust_height))
                        thrust_rotated = pygame.transform.rotate(thrust_image, -math.degrees(self.ship.angle))
                        thrust_rect = thrust_rotated.get_rect(center=(int(thrust_x), int(thrust_y)))
                        draw_surface.blit(thrust_rotated, thrust_rect)
                    except:
                        # Fallback to polygon flame
                        flame_points = []
                        for i in range(3):
                            angle = thrust_angle + (i - 1) * 0.3
                            x = thrust_x + math.cos(angle) * thrust_width / 2
                            y = thrust_y + math.sin(angle) * thrust_width / 2
                            flame_points.append((x, y))
                        pygame.draw.polygon(draw_surface, YELLOW, flame_points)
                self.ship.position.x = original_x
                self.ship.position.y = original_y
            ...
            if self.ship:
                ...
                if self.ship.image:
                    rotated_ship = pygame.transform.rotate(self.ship.image, -math.degrees(self.ship.angle) - 90)
                    ship_rect = rotated_ship.get_rect(center=(int(self.ship.position.x), int(self.ship.position.y)))
                    
                    if self.ship.invulnerable and int(self.ship.invulnerable_time * 40) % 2:
                        cyan_ship = self.ship.image.copy()
                        cyan_ship.fill((0, 255, 255, 128), special_flags=pygame.BLEND_MULT)
                        cyan_ship = pygame.transform.rotate(cyan_ship, -math.degrees(self.ship.angle) - 90)
                        draw_surface.blit(cyan_ship, ship_rect)
                    elif self.ship.red_flash_timer > 0:
                        red_ship = self.ship.image.copy()
                        red_ship.fill((255, 0, 0, 128), special_flags=pygame.BLEND_MULT)
                        red_ship = pygame.transform.rotate(red_ship, -math.degrees(self.ship.angle) - 90)
                        draw_surface.blit(red_ship, ship_rect)
                    else:
                        draw_surface.blit(rotated_ship, ship_rect)
                else:
                    # Fallback triangle
                    points = []
                    for i in range(3):
            ...
            # Top layer - Debug hitboxes if debug mode is enabled
            if self.debug_mode:
        ...
        # Draw progress bars (only during gameplay) (HIDDEN)
        # if self.game_state == "playing":
    ...
    def draw_debug_rate_of_fire_display(self, surface):
        ...
        base_interval = 0.09  # Base interval without bonus
        asteroid_bonus = self.ship.asteroid_interval_bonus
        shots_per_second = 1.0 / current_interval if current_interval > 0 else 0
        
        # Format rate of fire text (removed progress, peak, and shooting info)
        rof_text = f"ROF: {shots_per_second:.1f} shots/sec | Interval: {current_interval:.4f}s"
        bonus_text = f"Asteroid Bonus: -{asteroid_bonus:.4f}s | Base: {base_interval:.3f}s"
    ...
    def draw_debug_hitboxes(self, surface):
        ...
        for asteroid in self.asteroids:
            if asteroid.active:
                # Draw hitbox at offset position
                hitbox_center = asteroid.get_hitbox_center()
                self.draw_wrapped_hitbox(surface, hitbox_center, asteroid.radius, (0, 255, 0))
            ...
            for asteroid in self.asteroids[:]:
                ...
                if boss.polygon_circle_collision_with_wrapping(asteroid.get_hitbox_center().x, asteroid.get_hitbox_center().y, asteroid.radius, self.current_width, self.current_height):
                    ...
                    if asteroid.size >= 3:  # Sizes 3-9: Split the asteroid
                        ...
                        # Add score (size 3 = 300 points, size 4 = 400 points, etc.)
                        self.add_score(asteroid.size * 100)
        ...
        for new_asteroid in new_asteroids[:]:  # Use slice to avoid modification during iteration
            ...
            if boss.polygon_circle_collision_with_wrapping(new_asteroid.get_hitbox_center().x, new_asteroid.get_hitbox_center().y, new_asteroid.radius, self.current_width, self.current_height):
                ...
                if new_asteroid.size >= 3:  # Sizes 3-9: Split the asteroid
                    ...
                    # Add score (size 3 = 300 points, size 4 = 400 points, etc.)
                    self.add_score(new_asteroid.size * 100)
                    
                    # Split asteroid with 2x boss velocity
                    boss_velocity_2x = Vector2D(boss.velocity.x * 2, boss.velocity.y * 2)
                    even_newer_asteroids = new_asteroid.split(boss_velocity_2x)
                    self.asteroids.extend(even_newer_asteroids)
                ...
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        if event.key == pygame.K_SPACE and self.game_state == "waiting":
                            ...
                            # ESC key exits the game from any state
                            self.running = False
