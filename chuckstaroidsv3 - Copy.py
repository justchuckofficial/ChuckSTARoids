...
class Ship(GameObject):
    ...
    def draw(self, screen):
        ...
        if self.image:
            ...
            rotated_ship = image_cache.get_rotated_image(self.image, rotation_angle)
            ...
            # Draw shadow triangle first (behind the ship) - optimized
            shadow_points = []
            for i in range(3):
                angle = self.angle + i * (2 * math.pi / 3)
            ...
            shadow_result = ShadowSurfaceOptimizer.create_minimal_shadow_surface(shadow_points, 107)
            if shadow_result:
    ...
    def draw_ship_shadow(self, screen):
        ...
        if self.image:
            ...
            # Fallback to triangle shape - optimized
...
class Asteroid(GameObject):
    def __init__(self, x, y, size=3, level=1):
        ...
        # Calculate shadow probability based on level: (100 - (current level * 15))% chance, minimum 1%
        shadow_probability = max(0.01, (100 - (level * 15)) / 100.0)
    ...
    def draw_shadow_only(self, screen, screen_width=None, screen_height=None):
        ...
        for pos_x, pos_y in positions:
            if self.image:
                ...
                # Draw shadow polygon first (behind the asteroid) - optimized
                # Dynamic shadow size: (100% + (3 * size level)%) = 1.0 + (0.03 * size)
                shadow_scale = 1.0 + (0.03 * self.size)
                # Dynamic shadow offset: (10 * size level) pixels
                shadow_offset = 10 * self.size
                ...
                if shadow_alpha > 0:  # Only draw shadow if opacity > 0
                    ...
                    # Use optimized shadow surface creation
                    shadow_result = ShadowSurfaceOptimizer.create_minimal_shadow_surface(shadow_points, shadow_alpha)
                    if shadow_result:
                        shadow_surface, shadow_pos = shadow_result
                        screen.blit(shadow_surface, shadow_pos)
    ...
    def draw(self, screen, screen_width=None, screen_height=None):
        ...
        for pos_x, pos_y in positions:
            if self.image:
                # Draw asteroid using cached rotated image
...
class BossEnemy(GameObject):
    def __init__(self, x, y, direction="right", screen_width=1000, screen_height=750):
        ...
        if direction == "right":
            ...
            self.position.y = -300  # Start off-screen top
            self.position.x = random.uniform(100, screen_width - 100)  # Random horizontal position
            self.velocity = Vector2D(0, self.speed)
            self.flip_horizontal = False
        elif direction == "bottom":
            self.position.y = screen_height + 300  # Start off-screen bottom
            self.position.x = random.uniform(100, screen_width - 100)  # Random horizontal position
            self.velocity = Vector2D(0, -self.speed)
            self.flip_horizontal = False
            ...
            self.position.x = screen_width + 300  # Start off-screen right
            self.position.y = -300  # Start off-screen top
            self.velocity = Vector2D(-self.speed, self.speed)
    ...
    def update(self, dt, screen_width=None, screen_height=None, asteroids=None, player=None):
        ...
        if self.direction in ["left", "right"]:
            # Horizontal movement with sine wave Y offset
            self.position.x += self.velocity.x * dt
            self.position.y = self.center_y + sine_offset
        elif self.direction in ["top", "bottom"]:
            # Vertical movement with sine wave X offset
            self.position.y += self.velocity.y * dt
            self.position.x += sine_offset
        elif self.direction in ["top_right", "top_left", "bottom_right", "bottom_left"]:
            # Diagonal movement with sine wave perpendicular offset
            self.position.x += self.velocity.x * dt
            self.position.y += self.velocity.y * dt
            # Add perpendicular sine wave offset
            if self.direction in ["top_right", "bottom_right"]:
                self.position.x += sine_offset
        ...
        if self.direction == "right" and self.position.x > self.screen_width + 300:
            ...
            self.active = False
        elif self.direction == "top" and self.position.y > self.screen_height + 300:
            # Boss has moved completely off the bottom side
            self.active = False
        elif self.direction == "bottom" and self.position.y < -300:
            # Boss has moved completely off the top side
            self.active = False
        elif self.direction in ["top_right", "top_left", "bottom_right", "bottom_left"]:
            # For diagonal movement, check if boss is completely off-screen
            if (self.position.x > self.screen_width + 300 or self.position.x < -300 or
                self.position.y > self.screen_height + 300 or self.position.y < -300):
                self.active = False
...
class Game:
    ...
    def spawn_asteroids(self, count=None):
        ...
        if self.level >= 2:
            ...
            for _ in range(additional_count):
                ...
                x, y = self.get_edge_position()
                asteroid = Asteroid(x, y, random_size, self.level)
                self.asteroids.append(asteroid)
    ...
    def handle_input(self, dt):
        ...
        # C key to clear image cache (debug feature)
            ...
            for bullet in boss.weapon_bullets[:]:
                ...
                for asteroid in self.asteroids[:]:
                    ...
                    if self.check_wrapped_collision(bullet.position, asteroid.get_hitbox_center(), bullet.radius, asteroid.radius):
                        ...
                        # Add score (size 4 = 400 points, size 3 = 300, etc.)
                        self.asteroids_destroyed_this_level += 1  # Track asteroid destroyed by boss weapon
                        self.add_score(asteroid.size * 100)
    ...
    def draw(self, dt=0.016):
        ...
        if self.game_state == "playing":
            ...
            if self.debug_mode:
                ...
                self.draw_debug_cache_stats(draw_surface)
        else:
    ...
    def should_spawn_bosses(self):
        ...
        return self.level in [3, 6, 9] or self.level >= 12
    ...
    def spawn_bosses_for_level(self):
        ...
        if self.level == 3:
            ...
            self.spawn_two_bosses_opposite_sides()
            ...
            # Level 12+: 33% chance to spawn (current level/4 rounded down), 33% chance to spawn 4 corner bosses, 34% chance to spawn 2 bosses
            chance = random.random()
            if chance < 0.33:
                # 33% chance: (current level/4 rounded down) bosses
                num_bosses = max(1, self.level // 4)
                self.spawn_multiple_bosses_random_sides(num_bosses)
            elif chance < 0.66:
                # 33% chance: 4 corner bosses
    ...
    def spawn_two_bosses_opposite_sides(self):
        """Spawn 2 bosses on opposite sides, one on top one on bottom"""
        # First boss: top side
        boss1 = BossEnemy(0, 0, "top", self.current_width, self.current_height)
        self.bosses.append(boss1)
        self.check_boss_spawn_collisions(boss1)
        
        # Second boss: bottom side
        boss2 = BossEnemy(0, 0, "bottom", self.current_width, self.current_height)
    ...
    def spawn_four_corner_bosses(self):
        """Spawn 4 bosses in the corners: top right, top left, bottom right, bottom left"""
        corners = ["top_right", "top_left", "bottom_right", "bottom_left"]
        for corner in corners:
            boss = BossEnemy(0, 0, corner, self.current_width, self.current_height)
            self.bosses.append(boss)
            self.check_boss_spawn_collisions(boss)
