...
class GameLogger:
    ...
    def __init__(self):
        ...
        try:
            ...
            print(f"Warning: GameLogger initialization failed: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _write_high_scores_to_file(self):
        ...
        try:
            ...
            print(f"Warning: Could not write high scores to file: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _load_high_scores_from_header(self):
        ...
        try:
            ...
            print(f"Warning: Could not load high scores from header: {e}")
    ...
    def clear_gamelog_preserve_scores(self):
        ...
        try:
            ...
            try:
                ...
                print(f"Warning: Could not write to log file: {write_error}")
    ...
    def cleanup_gamelog_for_new_level(self, level, asteroids_destroyed, ufos_destroyed, current_score, current_multiplier):
        ...
        try:
            ...
            print(f"Warning: Could not clean up gamelog for new level: {e}")
            import traceback
            traceback.print_exc()
            # Fallback: just log the level completion without cleanup
            try:
            ...
            for i, score in enumerate(scores[:10], 1):
                try:
                    ...
                    if isinstance(score, dict):
                        ...
                        # Handle tuple/list format: (score, player_name, ...)
...
class Game:
            ...
            if self.should_check_collision('ship_asteroid', 1.0/60.0):
                for asteroid in self.asteroids:
                    ...
                    if self.check_wrapped_collision(self.ship.position, asteroid.get_hitbox_center(), self.ship.radius, asteroid.radius):
                        # Collision!
                        print(f"[DEBUG] Player hit by asteroid! Shield hits before: {self.ship.shield_hits}, Lives: {self.lives}")
                        if self.ship.shield_hits > 0:
                            # Shield absorbs hit
                            self.ship.shield_hits -= 1
                            print(f"[DEBUG] Shield absorbed hit! Shield hits after: {self.ship.shield_hits}")
                            ...
                            print(f"[DEBUG] Player destroyed by asteroid! No shields left. Lives remaining: {self.lives - 1}")
                            ...
                            asteroid.active = False
                            self.lives -= 1
                            print(f"[DEBUG] Lives after death: {self.lives}")
                            ...
                            if self.lives <= 0:
                                # Log player death
                                print(f"[DEBUG] Player died! Lives: {self.lives}, transitioning to death_delay state")
                                ...
                                print(f"[DEBUG] Player still alive, respawning ship. Lives: {self.lives}")
                                try:
                                    ...
                                    print(f"[DEBUG] Ship respawned successfully, game state: {self.game_state}")
                                except Exception as e:
                                    print(f"[DEBUG] Error during respawn: {e}")
                                    import traceback
                                    traceback.print_exc()
                        break
        ...
        if self.ship.active and not self.ship.invulnerable and not self.god_mode:
            for ufo in self.ufos:
                ...
                if self.check_wrapped_collision(self.ship.position, ufo.get_hitbox_center(), self.ship.radius, ufo.radius):
                    # Collision!
                    print(f"[DEBUG] Player hit by UFO! Shield hits before: {self.ship.shield_hits}, Lives: {self.lives}")
                    
                    # Check if UFO is in spinout mode
                    if ufo.spinout_active:
                        print(f"[DEBUG] UFO in spinout mode - instant damage!")
                        # UFO in spinout mode - damage player instantly
                        if self.ship.shield_hits > 0:
                            # Shield absorbs hit
                            self.ship.shield_hits -= 1
                            print(f"[DEBUG] Shield absorbed UFO hit! Shield hits after: {self.ship.shield_hits}")
                            ...
                            print(f"[DEBUG] Player destroyed by UFO (spinout mode)! No shields left. Lives remaining: {self.lives - 1}")
                    ...
                    try:
                        if hasattr(ufo, 'spinout_collision_delay_timer') and hasattr(ufo, 'spinout_collision_delay'):
                            if ufo.spinout_collision_delay_timer >= ufo.spinout_collision_delay:
                                ...
                                # Add score for UFO destruction after spinout
                                self.ufos_killed_this_level += 1
                                self.total_ufos_destroyed_this_level += 1  # Track UFO killed by player (spinout collision)
                        ...
                        self.ufos_killed_this_level += 1
                        self.total_ufos_destroyed_this_level += 1  # Track UFO killed by player (spinout collision exception)
                    ...
                    # If delay hasn't passed, UFO continues spinning but collision still occurs
                        ...
                        if self.ship.shield_hits > 0:
                            # Shield absorbs hit
                            self.ship.shield_hits -= 1
                            print(f"[DEBUG] Shield absorbed UFO hit (normal collision)! Shield hits after: {self.ship.shield_hits}")
                            ...
                            self.add_score(250, "ufo collision")  # 250 points for destroying UFO by collision
                            # Log UFO destruction
                            game_logger.log_ufo_destroyed("shield collision", self.level)
                        else:
                            # No shield, ship destroyed
                            print(f"[DEBUG] Player destroyed by UFO (normal collision)! No shields left. Lives remaining: {self.lives - 1}")
                            ...
                            ufo.active = False
                            self.ufos_killed_this_level += 1
                            self.total_ufos_destroyed_this_level += 1  # Track UFO killed by player (ship collision)
                            # Log UFO destruction
                            game_logger.log_ufo_destroyed("ship collision", self.level)
                            self.lives -= 1
        ...
        if self.ship.active and not self.ship.invulnerable and not self.god_mode:
            for bullet in self.ufo_bullets[:]:
                ...
                if self.check_wrapped_collision(self.ship.position, bullet.position, self.ship.radius, bullet.radius):
                    # Hit!
                    print(f"[DEBUG] Player hit by UFO bullet! Shield hits before: {self.ship.shield_hits}, Lives: {self.lives}")
                    bullet.active = False
                    ...
                    if self.ship.shield_hits > 0:
                        # Shield absorbs hit
                        self.ship.shield_hits -= 1
                        print(f"[DEBUG] Shield absorbed UFO bullet hit! Shield hits after: {self.ship.shield_hits}")
                        ...
                        # No shield, ship destroyed
                        print(f"[DEBUG] Player destroyed by UFO bullet! No shields left. Lives remaining: {self.lives - 1}")
            ...
            for bullet in boss.weapon_bullets[:]:
                ...
                if self.ship.active and not self.ship.invulnerable and not self.god_mode:
                    if self.check_wrapped_collision(bullet.position, self.ship.position, bullet.radius, self.ship.radius):
                        ...
                        print(f"[DEBUG] Player hit by boss bullet! Shield hits before: {self.ship.shield_hits}, Lives: {self.lives}")
                        bullet.active = False
                        ...
                        if self.ship.shield_hits > 0:
                            # Shield absorbs hit
                            self.ship.shield_hits -= 1
                            print(f"[DEBUG] Shield absorbed boss bullet hit! Shield hits after: {self.ship.shield_hits}")
                            ...
                            # No shields - ship destroyed
                            print(f"[DEBUG] Player destroyed by boss bullet! No shields left. Lives remaining: {self.lives - 1}")
    ...
    def update(self, dt):
        ...
        if self.game_state == "death_delay":
            print(f"[DEBUG] In death_delay state, timer: {self.death_delay_timer}")
        ...
        if self.game_state not in ["playing", "paused", "death_delay"]:
            print(f"[DEBUG] Game state not in valid states: {self.game_state}")
            return
    ...
    def draw(self, dt=0.016):
        ...
        if self.game_state == "waiting":
            ...
            controls_text2 = controls_font.render("SCORES - tab  .  RESTART - r  .  PAUSE - p  .  DATA - 0  .  EXIT - esc", True, (200, 200, 200))
        ...
        try:
            while self.running:
                ...
                try:
                                    ...
                                    if event.key == pygame.K_TAB:
                                        ...
                                        if self.scoreboard_available and self.scoreboard:
                                            ...
                                            if self.show_scoreboard:
                                                ...
                                                print(f"[SCOREBOARD DEBUG] Closing scoreboard on game over - keys_pressed state: {getattr(self, 'keys_pressed', 'NOT_SET')}")
                                                # Clear TAB key from keys_pressed to prevent movement conflicts
                                                if hasattr(self, 'keys_pressed'):
                                                    self.keys_pressed.discard(pygame.K_TAB)
                                        else:
                    ...
                    print(f"[DEBUG] Event handling exception: {e}")
                    import traceback
                    traceback.print_exc()
            ...
            print(f"[DEBUG] Game crashed with exception: {e}")
            import traceback
            traceback.print_exc()
            pygame.quit()
        finally:
            ...
            try:
                ...
                for i, score_line in enumerate(formatted_scores):
                    try:
                        ...
                        if len(score_line) > 50:
                            score_line = score_line[:47] + "..."
