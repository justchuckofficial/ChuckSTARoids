...
class Ship(GameObject):
    ...
    def apply_rotation(self, raw_dt, rotation_scale=1.0):
        ...
        if abs(self.target_rotation_speed) > 0:
            ...
            capped_dt = min(raw_dt, 1.0 / 30.0)  # Cap at 30 FPS equivalent
...
class BossEnemy(GameObject):
    ...
    def update(self, dt, screen_width=None, screen_height=None, asteroids=None, player=None):
        ...
        if self.direction in ["left", "right"]:
            ...
            self.position.y += self.velocity.y * dt
            # Store the initial X position for vertical movement to prevent drift
            if not hasattr(self, 'initial_x'):
                self.initial_x = self.position.x
            self.position.x = self.initial_x + sine_offset
        elif self.direction in ["top_right", "top_left", "bottom_right", "bottom_left"]:
            # Diagonal movement with sine wave perpendicular offset
            self.position.x += self.velocity.x * dt
            self.position.y += self.velocity.y * dt
            # Add perpendicular sine wave offset (this is fine for diagonal movement)
