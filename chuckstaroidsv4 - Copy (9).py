...
class GameLogger:
    ...
    def _write_log(self, message):
        ...
        if self.final_score_logged:
            print(f"DEBUG: Skipping log write - final_score_logged is True")
            return
            
        try:
            timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]  # Include milliseconds
            log_line = f"[{timestamp}] {message}\n"
            print(f"DEBUG: Writing to log file: {log_line.strip()}")
            # Always append to preserve existing content including high scores
            with open(self.log_file, 'a', encoding='utf-8') as f:
                f.write(log_line)
        except Exception as e:
    ...
    def log_score_event(self, points, event_type, total_score, multiplier, multiplier_gained):
        ...
        print(f"DEBUG: Writing to game log: {log_message}")
...
class Game:
    ...
    def add_ufo(self, ufo):
        ...
        print(f"DEBUG: Created UFO at ({ufo.position.x:.1f}, {ufo.position.y:.1f}) - Total UFOs: {len(self.ufos)}")
    ...
    def add_score(self, points, event_type=""):
        ...
        print(f"DEBUG: add_score - points: {multiplied_points}, event_type: '{event_type}', total: {self.score}")
    ...
    def shoot(self):
        ...
        try:
            ...
            print(f"DEBUG: Created bullet at ({bullet_x:.1f}, {bullet_y:.1f}) - Total bullets: {len(self.bullets)}")
    ...
    def check_collisions(self):
        ...
        if self.should_check_collision('bullet_asteroid', 1.0/60.0):
            for bullet in self.bullets[:]:
                if not bullet.active:
                    continue
                # Debug: Check if bullet is still active after UFO collision
                if hasattr(bullet, '_hit_ufo') and bullet._hit_ufo:
                    print(f"DEBUG: Bullet that hit UFO is now checking asteroid collision - bullet.active: {bullet.active}")
                for asteroid in self.asteroids[:]:
        ...
        should_check = self.should_check_collision('bullet_ufo', 1.0/60.0)
        if should_check:
            active_bullets = [b for b in self.bullets if b.active]
            active_ufos = [u for u in self.ufos if u.active]
            if active_bullets and active_ufos:
                print(f"DEBUG: Checking {len(active_bullets)} bullets vs {len(active_ufos)} UFOs")
            for bullet in self.bullets[:]:
                if not bullet.active:
                    continue
                for ufo in self.ufos[:]:
                    ...
                    if self.check_wrapped_collision(bullet.position, ufo.get_hitbox_center(), bullet.radius, ufo.radius):
                        # Hit!
                        print(f"DEBUG: Bullet hit UFO! UFO spinout_active: {ufo.spinout_active}")
                        print(f"DEBUG: Deactivating bullet - was active: {bullet.active}")
                        bullet.active = False
                        bullet._hit_ufo = True  # Flag to track this bullet hit a UFO
                        print(f"DEBUG: Bullet deactivated - now active: {bullet.active}")
                        ...
                        if ufo.spinout_active:
                            ...
                            if hasattr(ufo, 'spinout_collision_delay_timer') and hasattr(ufo, 'spinout_collision_delay'):
                                print(f"DEBUG: UFO in spinout - delay_timer: {ufo.spinout_collision_delay_timer}, delay: {ufo.spinout_collision_delay}")
                                if ufo.spinout_collision_delay_timer >= ufo.spinout_collision_delay:
                                    ...
                                    # Log UFO destruction
                                    print(f"DEBUG: Logging UFO destruction - spinout delay passed")
                        ...
                        print(f"DEBUG: Level {self.level}, spinout_chance: {spinout_chance}, random: {random.random():.3f}")
                        if random.random() < spinout_chance:
                            ...
                            # Normal destruction
                            try:
                                print(f"DEBUG: UFO normal destruction (not in spinout)")
                                ufo.active = False
                                print(f"DEBUG: UFO deactivated, proceeding with destruction effects")
                                
                                # Add shot hit particles
                                print(f"DEBUG: Adding shot hit particles")
                                self.explosions.add_shot_hit_particles(ufo.position.x, ufo.position.y)
                                
                                # Add screen shake for UFO destruction
                                print(f"DEBUG: Adding screen shake")
                                ...
                                print(f"DEBUG: Adding {total_particles} explosion particles")
                                ...
                                print(f"DEBUG: About to add UFO kill tracking and score")
                                self.ufos_killed_this_level += 1
                                self.total_ufos_destroyed_this_level += 1  # Track UFO killed by player
                                print(f"DEBUG: About to add score for UFO kill")
                                self.add_score(500, "ufo shot")  # 500 points for destroying UFO with shot
                                # Log UFO destruction
                                print(f"DEBUG: Logging UFO destruction - normal destruction")
                                game_logger.log_ufo_destroyed("player shot", self.level)
                                print(f"DEBUG: UFO destruction completed successfully")
                            except Exception as e:
                                print(f"DEBUG: Exception in UFO destruction: {e}")
                                import traceback
                                traceback.print_exc()
    ...
    def update(self, dt):
        ...
        if self.game_state == "death_delay":
            ...
            if self.death_delay_timer >= 2.0:
                ...
                if self.scoreboard_available and self.scoreboard:
                    self.load_scoreboard_background()
                    print(f"[SCOREBOARD DEBUG] Scoreboard available, loading scores in background")
                else:
                    print(f"[SCOREBOARD DEBUG] Scoreboard not available - available: {self.scoreboard_available}, scoreboard: {self.scoreboard is not None}")
                
                # Automatically trigger name input if score qualifies
                print(f"[SCOREBOARD DEBUG] Game over - Final score: {self.score}")
                if self.score > 0:  # Any positive score qualifies for name input
                    ...
                    print(f"[SCOREBOARD DEBUG] Score too low for name input: {self.score}")
    ...
    def advance_level(self):
        ...
        if hasattr(self, 'pre_ability_asteroids_destroyed') and hasattr(self, 'pre_ability_ufos_killed'):
            ...
            print(f"DEBUG: Using pre-ability counts - Pre: A={self.pre_ability_asteroids_destroyed}, U={self.pre_ability_total_ufos_destroyed}, Current: A={self.asteroids_destroyed_this_level}, U={self.total_ufos_destroyed_this_level}, Final: A={asteroids_destroyed}, U={ufos_destroyed}")
            ...
            print(f"DEBUG: Using current counts - A={asteroids_destroyed}, U={ufos_destroyed}")
