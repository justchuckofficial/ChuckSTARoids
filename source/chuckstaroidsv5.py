...
class GameLogger:
    ...
    def __init__(self):
        ...
        self.high_scores = []  # List to store high scores with timestamps and levels
        self.final_score_logged = False  # Flag to prevent logging after final score
        try:
            self._ensure_log_file()
            self._load_high_scores()
        except Exception as e:
            # GameLogger initialization failed - silent error handling
            # Ensure we have a valid state even if initialization fails
            self.high_scores = []
            self.final_score_logged = False
    ...
    def _ensure_log_file(self):
        ...
        try:
            ...
            # Could not create log file - silent error handling
            pass
    
    def _write_log(self, message):
        """Write a timestamped message to the log file (always preserves high scores)"""
        # Don't log anything after final score has been logged
        if self.final_score_logged:
            return
        ...
        try:
            ...
            # Could not write to log file - silent error handling
            pass
    ...
    def log_score_event(self, points, event_type, total_score, multiplier, multiplier_gained):
        ...
        log_message = f"SCORE: +{points} points ({event_type}) - Total: {total_score} x{multiplier:.2f}"
    ...
    def log_game_start(self):
        """Log game start"""
        # Reset the flag to allow logging for new game
        self.final_score_logged = False
    ...
    def log_game_over(self, final_score, final_level):
        ...
        self._write_log(f"FINAL SCORE: {final_score} - Level {final_level}")
        # Add score to high scores list with level
        self.add_high_score(final_score, final_level)
        # Immediately write high scores to file to ensure they're saved
        self._write_high_scores_to_file()
        # Set flag to prevent further logging
        self.final_score_logged = True
    
    def log_ability_use(self, ability_type):
        """Log ability usage"""
        self._write_log(f"ABILITY USED: {ability_type}")
    
    def log_ufo_destroyed(self, destruction_method, level):
        """Log when a UFO is destroyed by the player"""
        self._write_log(f"UFO DESTROYED: {destruction_method} on level {level}")
    
    def _load_high_scores(self):
        """Load existing high scores with timestamps and levels from the main log file"""
        from datetime import datetime
        try:
            # First, try to load from the header section (formatted high scores)
            header_scores = self._load_high_scores_from_header()
            if header_scores:
                self.high_scores = header_scores
                return
            
            # If no header scores found, fall back to loading from FINAL SCORE entries
            if os.path.exists(self.log_file):
                with open(self.log_file, 'r', encoding='utf-8') as f:
                    ...
                    for line in lines:
                        ...
                        if "FINAL SCORE:" in line:
                            # Extract score, level, and timestamp from the line
                            try:
                                ...
                                # Parse score and level from "score - Level level" format
                                score_level = 1  # Default level if not found
                                if " - Level " in score_part:
                                    score_str, level_str = score_part.split(" - Level ")
                                ...
                                # Store as tuple (score, timestamp, level)
                                self.high_scores.append((score, timestamp or datetime.now(), level))
                ...
                seen_scores = set()
                unique_scores = []
                for score_data in self.high_scores:
                    if len(score_data) == 3:
                        score, timestamp, score_level = score_data
                    else:
                        # Legacy format - default to level 1
                        score, timestamp = score_data
                        score_level = 1
                    
                    if score not in seen_scores:
                        seen_scores.add(score)
                        unique_scores.append((score, timestamp, level))
                
                self.high_scores = sorted(unique_scores, key=lambda x: x[0], reverse=True)[:10]
        except Exception as e:
            # Could not load high scores - silent error handling
            pass
            self.high_scores = []
    
    def add_high_score(self, score, level):
        """Add a new high score with current timestamp and level"""
        if score > 0:
            from datetime import datetime
            self.high_scores.append((score, datetime.now(), level))
            # Remove duplicates and sort by score, keep top 10
            # Use a set to remove duplicates based on score only
            seen_scores = set()
            unique_scores = []
            for score, timestamp, level in self.high_scores:
                if score not in seen_scores:
                    seen_scores.add(score)
                    unique_scores.append((score, timestamp, level))
            self.high_scores = sorted(unique_scores, key=lambda x: x[0], reverse=True)[:10]
    
    def _write_high_scores_to_file(self):
        """Write current high scores to the file header immediately"""
        from datetime import datetime
        try:
            ...
            # Find where the high scores section ends
            lines = existing_content.split('\n')
            new_content_lines = []
            in_high_scores = False
            high_scores_written = False
            
            for line in lines:
                if line.strip() == "=== CHUCKSTEROIDS HIGH SCORES ===":
                    in_high_scores = True
                    # Write updated high scores
                    new_content_lines.append("=== CHUCKSTEROIDS HIGH SCORES ===")
                    if self.high_scores:
                        for i, (score, timestamp, score_level) in enumerate(self.high_scores, 1):
                            timestamp_str = timestamp.strftime("%m/%d/%Y %H:%M")
                            new_content_lines.append(f"{i:2d}. {score:,} - Level {score_level} - {timestamp_str}")
                    else:
                        new_content_lines.append("No scores yet")
                    new_content_lines.append("=" * 30)
                    high_scores_written = True
                    continue
                elif line.strip() == "=" * 30 and in_high_scores:
                    in_high_scores = False
                    continue
                elif not in_high_scores:
                    new_content_lines.append(line)
            
            # If no high scores section was found, add it at the beginning
            if not high_scores_written:
                header_lines = ["=== CHUCKSTEROIDS HIGH SCORES ==="]
                if self.high_scores:
                    for i, (score, timestamp, score_level) in enumerate(self.high_scores, 1):
                        timestamp_str = timestamp.strftime("%m/%d/%Y %H:%M")
                        header_lines.append(f"{i:2d}. {score:,} - Level {score_level} - {timestamp_str}")
                else:
                    header_lines.append("No scores yet")
                header_lines.append("=" * 30)
                header_lines.append("")
                
                # Combine header with existing content
                new_content = '\n'.join(header_lines + new_content_lines)
            else:
            ...
            # Could not write high scores to file - silent error handling
            pass
    ...
    def _load_high_scores_from_header(self):
        """Load high scores from the file header section"""
        from datetime import datetime
        try:
            if os.path.exists(self.log_file):
                with open(self.log_file, 'r', encoding='utf-8') as f:
                ...
                for line in lines:
                    ...
                    # Check if we're in the high scores section
                    if line == "=== CHUCKSTEROIDS HIGH SCORES ===":
                        in_header = True
                        continue
                    elif line == "==============================":
                        in_header = False
                    ...
                    # If we're in the header and this looks like a score line
                    if in_header and line and not line.startswith("No scores yet"):
                        # Parse score line format: " 1. 401,548 - Level 5 - 09/14/2025 02:37"
                        if ". " in line and " - " in line:
                            try:
                                # Extract score, level, and timestamp
                                parts = line.split(" - ")
                                if len(parts) >= 2:
                                    score_part = parts[0].split(". ", 1)[1].replace(",", "")
                                    score = int(score_part)
                                    
                                    # Check if level information is present
                                    score_level = 1  # Default level
                                    if len(parts) == 3 and parts[1].startswith("Level "):
                                        # New format with level: "Level 5 - 09/14/2025 02:37"
                                        level_str = parts[1].replace("Level ", "")
                                        level = int(level_str)
                                        timestamp_str = parts[2]
                                    else:
                                        # Legacy format without level: "09/14/2025 02:37"
                                    ...
                                    high_scores.append((score, timestamp, level))
                            except (ValueError, IndexError):
                                continue
                
                return high_scores
        except Exception as e:
            # Could not load high scores from header - silent error handling
            pass
    ...
    def clear_gamelog_preserve_scores(self):
        ...
        from datetime import datetime
        try:
            # Load high scores from the current file header
            header_scores = self._load_high_scores_from_header()
            
            # Always reload scores from FINAL SCORE entries to catch new scores
            self._ensure_high_scores_preserved()
            final_score_entries = self.high_scores.copy()
            
            # Merge scores from both sources, removing duplicates
            all_scores = []
            seen_scores = set()
            
            # Add header scores first
            for score_data in header_scores:
                if len(score_data) == 3:
                    score, timestamp, score_level = score_data
                else:
                    # Legacy format - default to level 1
                ...
                if score not in seen_scores:
                    seen_scores.add(score)
                    all_scores.append((score, timestamp, score_level))
            
            # Add final score entries, avoiding duplicates
            for score_data in final_score_entries:
                if len(score_data) == 3:
                    score, timestamp, score_level = score_data
                else:
                    # Legacy format - default to level 1
            ...
            existing_high_scores = sorted(all_scores, key=lambda x: x[0], reverse=True)[:10]
            
            # Update in-memory high scores to reflect the merged list
            self.high_scores = existing_high_scores
            
            # Create new content with high scores at the top
            new_content = "=== CHUCKSTEROIDS HIGH SCORES ===\n"
            if existing_high_scores:
                for i, (score, timestamp, score_level) in enumerate(existing_high_scores, 1):
                    # Format timestamp as MM/DD/YYYY HH:MM
                    timestamp_str = timestamp.strftime("%m/%d/%Y %H:%M")
                    new_content += f"{i:2d}. {score:,} - Level {score_level} - {timestamp_str}\n"
            else:
            ...
            try:
                ...
                # Could not write to log file - silent error handling
                # Try to create a minimal log file
                try:
                    with open(self.log_file, 'w', encoding='utf-8') as f:
                        f.write("=== CHUCKSTEROIDS HIGH SCORES ===\nNo scores yet\n==============================\n\n")
                except:
                    pass
                
        except Exception as e:
            # Could not clear main log while preserving scores - silent error handling
            pass
    ...
    def get_top_local_score_with_level(self):
        """Get the top local score and level from high_scores"""
        if not self.high_scores:
            return None, None
        try:
            # Return both score and level (first in sorted list)
            score, timestamp, level = self.high_scores[0]
            return score, level
        except (IndexError, TypeError, ValueError):
            return None, None
    
    def cleanup_gamelog_for_new_level(self, level, asteroids_destroyed, ufos_destroyed, current_score, current_multiplier):
        """Surgically clean up gamelog when a new level is loaded
        
        Preserves:
        - High scores section at the top
        - All level completion summaries from current game session
        - Level completion summary line with format: Level # Complete: (asteroids) (ufos) (score)x(multiplier)
        
        Removes:
        - All other detailed game events from the previous level
        """
        from datetime import datetime
        try:
            # Load high scores from the current file header
            ...
            # Extract existing level summaries from current log file
            existing_level_summaries = []
            try:
                with open(self.log_file, 'r', encoding='utf-8') as f:
                    lines = f.readlines()
                    for line in lines:
                        if "Level" in line and "Complete:" in line and "asteroids destroyed by player" in line:
                            existing_level_summaries.append(line.strip())
            except Exception:
                # If we can't read the file, just continue without existing summaries
                pass
            ...
            # Add all existing level summaries from current session
            if existing_level_summaries:
                new_content += "=== LEVEL COMPLETION SUMMARIES ===\n"
                for summary in existing_level_summaries:
                    new_content += summary + "\n"
                new_content += "\n"
            
            # Add current level completion summary
            timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
            new_content += f"[{timestamp}] Level {level} Complete: {asteroids_destroyed} asteroids destroyed by player, {ufos_destroyed} UFOs destroyed by player, {current_score:,}x{current_multiplier:.2f}\n\n"
            
            # Add session header for new level
            new_content += f"{'='*60}\n"
            new_content += f"NEW LEVEL SESSION STARTED: Level {level + 1} - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
            new_content += f"{'='*60}\n"
            ...
            try:
                ...
                # Could not write to log file - silent error handling
                # Fallback: just log the level completion without cleanup
                try:
                    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
                    self._write_log(f"Level {level} Complete: {asteroids_destroyed} asteroids destroyed by player, {ufos_destroyed} UFOs destroyed by player, {current_score:,}x{current_multiplier:.2f}")
                except:
                    pass
                
        except Exception as e:
            # Could not clean up gamelog for new level - silent error handling
            pass
            # Fallback: just log the level completion without cleanup
            try:
                timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
...
def setup_logging():
    ...
    # Disable all logging for clean executable
...
MAX_PARTICLES = 2500  # Global particle limit for performance
PARTICLE_SOFT_LIMIT = 1250  # Start reducing particle generation (50% of original)
...
class Ship(GameObject):
    ...
    def update(self, dt, screen_width=None, screen_height=None, time_dilation_factor=1.0, raw_dt=None, multiplier=1.0, asteroid_count=0, level=1):
        # Only update if ship is active
        if not self.active:
            return
            
        # Store the current level for ability charging logic
        self.level = level
        ...
        self.update_rate_of_fire(dt, is_shooting, multiplier)
        ...
        if self.ability_charges < self.max_ability_charges and not self.ability_used:
            ...
            if self.ability_charges == 0 and self.is_first_game and self.level == 1:
                # First charge of first game on level 1: 5 seconds
        ...
        # Apply rotation smoothing with fixed speed (use raw frame time, not dilated time)
        # Turn rate is now consistent regardless of asteroid count
        self.apply_rotation(raw_dt, 1.0)
    ...
    def draw_ability_rings(self, screen):
        ...
        if self.ability_charges < self.max_ability_charges or self.ability_recharge_pulse_timer > 0 or self.ability_hold_timer > 0 or self.ability_fade_timer > 0 or (self.ability_charges == self.max_ability_charges and self.ability_fade_timer <= 0):
            ...
            # Charging phase
                ...
                if self.ability_charges == 0:
                    charge_duration = self.first_charge_duration if (self.is_first_game and self.level == 1) else self.ability_duration
                ...
                for charge in range(self.max_ability_charges):
                    ...
                    if is_ready:
                        ...
                        if self.ability_charges < self.max_ability_charges:  # Show progress if not fully charged
                            ...
                            if charge == 0:
                                # First ring: use first charge duration for first game on level 1, otherwise normal
                                charge_duration = self.first_charge_duration if (self.is_first_game and self.level == 1) else self.ability_duration
                                ability_progress = self.ability_timer / charge_duration
                            else:
    ...
    def get_effective_peak_time(self, current_multiplier):
        """Calculate effective peak time based on current multiplier
        Linear interpolation: 0.5s at 1x → 0.01s at 5x multiplier"""
        # Clamp multiplier between 1.0 and 5.0
        clamped_multiplier = max(1.0, min(5.0, current_multiplier))
        
        # Linear interpolation formula
        # At 1.0x: peak_time = 0.5s
        # At 5.0x: peak_time = 0.01s
        # Slope = (0.01 - 0.5) / (5.0 - 1.0) = -0.1225
        # peak_time = 0.5 + (-0.1225) * (multiplier - 1.0)
        peak_time = 0.5 - 0.1225 * (clamped_multiplier - 1.0)
        
        return max(0.01, peak_time)  # Never go below 0.01s

    def update_rate_of_fire(self, dt, is_shooting, current_multiplier=1.0):
        """Update rate of fire with dynamic peak time based on multiplier"""
        if is_shooting:
            # Only progress the curve when actively shooting
            self.rof_progression_time += dt
            
            # Calculate dynamic peak time based on current multiplier
            effective_peak_time = self.get_effective_peak_time(current_multiplier)
            
            # Check if we just reached the peak
            if self.rof_progression_time >= effective_peak_time and not self.rof_peak_reached:
                self.rof_peak_reached = True
                # Return True to indicate peak was reached (for spark effect)
                return True
            
            if self.rof_progression_time <= effective_peak_time:
                # Aggressive ramp up phase: 0.09s to 0.042s over dynamic time
                progress = self.rof_progression_time / effective_peak_time
                ...
                remaining_time = self.rof_curve_duration - effective_peak_time
                progress = (self.rof_progression_time - effective_peak_time) / remaining_time
...
class BossWeaponBullet(GameObject):
    ...
    def __init__(self, x, y, vx, vy, angle=None):
        ...
        # 7-second lifespan for boss bullets
        self.lifespan = 7.0  # 7 seconds
        self.age = 0.0  # Track bullet age
    ...
    def update(self, dt, screen_width=None, screen_height=None):
        # Update bullet age and check lifespan
        self.age += dt
        ...
        if self.size == 2:  # XXS asteroid
            if random.random() < 0.25:  # 25% chance to split into 2 XXXS
                ...
                for i in range(2):
                    ...
                    # 0.1% chance to add show_circle tag to new asteroid
                    if random.random() < 0.001:
                        new_asteroid.tags.append("show_circle")
...
class AdvancedUFO(GameObject):
    def __init__(self, x, y, ai_personality="aggressive"):
        ...
        self.spinout_duration = random.uniform(1.5, 3.0)  # Random 1.5-3 seconds duration
        ...
        # Spawn immunity system - immune to asteroids and other UFOs for 2 seconds after spawning
        self.spawn_immunity_timer = 2.0  # 2 seconds of spawn immunity
        self.spawn_immunity_active = True  # Initially active when UFO spawns
    ...
    def update(self, dt, ship_pos, screen_width=None, screen_height=None, time_dilation_factor=1.0, explosion_system=None):
        ...
        if self.spawn_immunity_active:
            self.spawn_immunity_timer -= dt * time_dilation_factor
            if self.spawn_immunity_timer <= 0:
                self.spawn_immunity_active = False
...
class AbilityUFO(AdvancedUFO):
    ...
    def update(self, dt, ship_pos, screen_width=None, screen_height=None, time_dilation_factor=1.0, explosion_system=None):
        ...
        super().update(dt, ship_pos, screen_width, screen_height, time_dilation_factor, explosion_system)
...
class BossEnemy(GameObject):
    def __init__(self, x, y, direction="right", screen_width=1000, screen_height=750, level=3):
        ...
        self.level = level  # Store level for targeting calculations
        ...
        self.weapon_delay = 0.0  # No delay - start shooting immediately upon spawn
        ...
        self.shot_timer = 0.0
        self.shot_interval = 0.25  # Base interval, will be updated based on level
        self.current_gun_index = 0
        self.shot_count = 0  # Track shots fired for player targeting
        self.asteroid_targets = []
        self.weapon_bullets = []
        self.all_at_once_fired = False  # Track if all-at-once mode has fired once
    ...
    def update(self, dt, screen_width=None, screen_height=None, asteroids=None, player=None):
        ...
        # Bosses become inactive at 300 pixels beyond screen width
    ...
    def update_shot_interval_by_level(self):
        """Update shot interval based on boss level"""
        if self.level <= 3:
            self.shot_interval = 0.25  # Levels 1-3: 0.25 seconds
        elif self.level <= 6:
            self.shot_interval = 0.20  # Levels 4-6: 0.20 seconds
        elif self.level <= 11:
            self.shot_interval = 0.15  # Levels 7-11: 0.15 seconds
        else:
            self.shot_interval = 0.10  # Levels 12+: 0.10 seconds
    ...
    def update_weapon_system(self, dt, asteroids, player):
        ...
        # Stop firing when boss is 100 pixels beyond screen width
        if self.direction in ["right", "right_top", "right_bottom"] and self.position.x > self.screen_width + 100:
            return  # Stop firing when boss is 100 pixels beyond screen width
        elif self.direction in ["left", "left_top", "left_bottom"] and self.position.x < -100:
            return  # Stop firing when boss is 100 pixels beyond screen width
        ...
        # Update shot interval based on level
        self.update_shot_interval_by_level()
        ...
        if self.cycle_timer >= self.cycle_duration:
            ...
            # All-at-once mode occurs every 3rd cycle (cycles 2, 5, 8, etc.)
            if self.cycle_count % 3 == 2:
                self.current_cycle = 1  # All-at-once mode
                self.all_at_once_fired = False  # Reset all-at-once flag
            else:
        ...
        # Calculate level-based targeting ratio: ((level/7, rounded down) + 1) out of 5 shots
        target_player_ratio = (self.level // 7) + 1  # Number of shots out of 5 that target player
        target_player_interval = 5 // target_player_ratio if target_player_ratio > 0 else 5  # Calculate interval
        
        # Fire shots based on current cycle
        if self.current_cycle == 0:  # One-at-a-time mode
            if self.shot_timer >= self.shot_interval:
                # Level-based targeting: every Nth shot targets the player
                if self.shot_count % target_player_interval < target_player_ratio and player:
        ...
        else:  # All-at-once mode - only fire once per cycle
            if not self.all_at_once_fired and self.shot_timer >= self.shot_interval:
                # Fire from all guns simultaneously (only once per cycle)
                num_guns = len(self.gun_positions)
                for gun_index in range(num_guns):
                    # All shots in all-at-once mode target the player
                    if player:
                        self.fire_weapon_shot_at_player(player, gun_index)
                    self.shot_count += 1
                
                self.all_at_once_fired = True  # Mark as fired for this cycle
                self.shot_timer = 0.0
                
                # Immediately switch back to one-at-a-time mode after firing all shots
                self.current_cycle = 0  # Switch to one-at-a-time mode
                self.current_gun_index = 0  # Reset gun index
    ...
    def add_explosion(self, x, y, num_particles=50, color=(255, 255, 0), asteroid_size=None, is_ufo=False, use_raw_time=False):
        ...
        for _ in range(int(num_particles)):
            ...
            if asteroid_size is not None:
                ...
                angle = random.uniform(0, math.pi/4)  # Random direction 0-45 degrees
            else:
            ...
            if asteroid_size is not None:
                ...
                # UFO explosion properties: 0.5-1.5 seconds (randomized), 1.0-3.0 pixels
                lifetime = random.uniform(0.5, 1.5)  # 0.5-1.5 seconds (randomized)
...
class Scoreboard:
    ...
    def get_scores(self):
        """Get top 100 scores from the API and display all 100 in game"""
        try:
            ...
            # Fetch fresh scores - request top 10 for display (API can handle 100)
            response = requests.get(
                f"{self.api_url}?action=get_scores&limit=10",
    ...
    def submit_score(self, player_name, score, level=1, game_instance=None):
        """Submit a new score to the API with bulletproof submission handling"""
        try:
            # Use game instance for submission state if provided
            if game_instance:
                # Check if submission is already in progress or completed
                if game_instance.submission_lock:
                    return False, "Submission already in progress"
                
                if game_instance.score_submission_completed:
                    return False, "Score already submitted for this game"
                
                # Check if we've exceeded max attempts
                if game_instance.score_submission_attempts >= game_instance.max_submission_attempts:
                    return False, "Maximum submission attempts exceeded"
                
                # Set submission lock
                game_instance.submission_lock = True
                game_instance.score_submission_in_progress = True
                game_instance.submission_start_time = time.time()
                game_instance.score_submission_attempts += 1
            
            # Validate inputs
            if not player_name or not isinstance(player_name, str):
                if game_instance:
                    self._reset_submission_state(game_instance)
            ...
            data = {
                ...
                'level': int(level)   # Add level information
            }
            ...
            if result.get('success'):
                ...
                if game_instance:
                    game_instance.score_submission_completed = True
                    self._reset_submission_state(game_instance)
    ...
    def _reset_submission_state(self, game_instance=None):
        """Reset submission state while preserving completion status"""
        try:
            if game_instance:
                game_instance.submission_lock = False
                game_instance.score_submission_in_progress = False
                game_instance.submission_start_time = 0.0
        except Exception as e:
            pass  # Silent error handling
    
    def submit_score_async(self, player_name, score, level=1, callback=None, game_instance=None):
        """Submit score in background thread with retry logic"""
        try:
            # Check if submission is already in progress or completed
            if game_instance and (game_instance.submission_lock or game_instance.score_submission_completed):
                if callback:
                    callback(False, "Submission already in progress or completed")
                return
            
            def _submit():
                try:
                    success, message = self.submit_score(player_name, score, level, game_instance)
                    
                    # If submission failed and we haven't exceeded max attempts, retry with exponential backoff
                    if not success and game_instance and game_instance.score_submission_attempts < game_instance.max_submission_attempts:
                        # Calculate retry delay (exponential backoff: 1s, 2s, 4s)
                        retry_delay = min(2 ** (game_instance.score_submission_attempts - 1), 8)
            ...
            for i, score in enumerate(scores[:10], 1):
                try:
                    ...
                    if isinstance(score, dict):
                        player = score.get('playerName', score.get('player_name', 'Unknown'))[:15]
                        score_val = score.get('score', score.get('points', 0))
                        level = score.get('level', score.get('level_reached', 1))
                    elif isinstance(score, (list, tuple)) and len(score) >= 2:
                        # Handle tuple/list format: (score, player_name, level, ...)
                        player = str(score[1])[:15] if len(score) > 1 else 'Unknown'
                        score_val = score[0] if len(score) > 0 else 0
                        level = score[2] if len(score) > 2 else 1  # Level is third element if available
                    else:
                    ...
                    # Ensure score_val and level are numbers
                    try:
                    ...
                    formatted.append(f"{i:2d}. {player:<15} {score_val:,} (Lv.{level})")
                    
                except Exception as e:
                    formatted.append(f"{i:2d}. Error loading score")
                    
        except Exception as e:
            return ["Error formatting scores"]
        
        return formatted if formatted else ["No valid scores found"]
...
class Game:
    def __init__(self):
        ...
        self.score = 0
        self.lives = 3
        self.max_lives = 3  # Maximum lives based on milestones (starts at 3, increases by 1 every 250k up to 5)
        self.level = 1
        ...
        try:
            ...
            self.scoreboard = Scoreboard('https://script.google.com/macros/s/AKfycbxXmf63amjtFQWQ_cfaS-xCzTOmjIPzSUvs7XmV2BRPvVsGQp7hU-fc1uEbgzgYO7Co-Q/exec')
            self.scoreboard_available = True
        except Exception as e:
            # Failed to initialize scoreboard - silent error handling
            pass
            self.scoreboard = None
            self.scoreboard_available = False
        
        self.show_scoreboard = False
        self.scoreboard_ever_opened = False  # Track if scoreboard has ever been opened on title screen
        self.player_name_input = ""
        self.name_input_active = False
        self.scoreboard_scores = []
        self.scoreboard_loading = False  # Track if scores are being loaded
        self.scoreboard_load_start_time = 0.0  # Track when loading started
        
        # Score submission state tracking for bulletproof submission
        self.score_submission_in_progress = False  # Track if a submission is currently happening
        self.score_submission_completed = False  # Track if score has been submitted for this game
        self.score_submission_attempts = 0  # Track number of submission attempts
        self.max_submission_attempts = 3  # Maximum retry attempts
        self.submission_lock = False  # Lock to prevent multiple simultaneous submissions
        self.submission_start_time = 0.0  # Track when submission started
        self.submission_timeout = 30.0  # Timeout for submission (30 seconds)
        
        # Score milestone tracking for rewards
        self.last_milestone_25k = 0  # Last 25k milestone reached (shield recharge)
        self.last_milestone_100k = 0  # Last 100k milestone reached (ability + shield recharge)
        self.last_milestone_250k = 0  # Last 250k milestone reached (extra life + shield + ability recharge)
        ...
        self.star_explosion_duration = 0.8  # 0.8 seconds (0.25 explosion + 0.15 fade + 0.4 fade-in)
        ...
        self.total_ufos_destroyed_this_level = 0  # Total UFOs destroyed for level summary (never reset)
        ...
        self.show_25k_message = False
        self.show_100k_message = False
        self.show_250k_message = False
        self.message_25k_timer = 0.0
        self.message_100k_timer = 0.0
        self.message_250k_timer = 0.0
        
        # Lowest priority UI messages (UFO count and multiplier)
        self.show_ufo_10_message = False
        self.show_ufo_20_message = False
        self.show_ufo_30_message = False
        self.show_ufo_60_message = False
        self.show_ufo_90_message = False
        self.show_ufo_too_many_message = False
        self.show_mult_3x_message = False
        self.show_mult_4x_message = False
        self.show_mult_5x_message = False
        self.show_nice_shot_message = False
        
        self.ufo_10_timer = 0.0
        self.ufo_20_timer = 0.0
        self.ufo_30_timer = 0.0
        self.ufo_60_timer = 0.0
        self.ufo_90_timer = 0.0
        self.ufo_too_many_timer = 0.0
        self.mult_3x_timer = 0.0
        self.mult_4x_timer = 0.0
        self.mult_5x_timer = 0.0
        self.nice_shot_timer = 0.0
        
        # Track UFO counts for messages
        self.ufos_spawned_this_level = 0
        self.ufo_type_counts = {}  # Track count of each UFO type
        self.last_multiplier_threshold = 0  # Track highest multiplier threshold reached
        
        # Track which UFO count messages have been shown this level
        self.ufo_10_shown = False
        self.ufo_20_shown = False
        self.ufo_30_shown = False
        self.ufo_60_shown = False
        self.ufo_90_shown = False
        self.ufo_too_many_shown = False
        ...
        # Load controls image
        try:
            self.controls_image = pygame.image.load(get_resource_path("controls.gif"))
            self.controls_image = self.controls_image.convert_alpha()
            # Scale to 650 pixels wide while maintaining aspect ratio
            original_width = self.controls_image.get_width()
            original_height = self.controls_image.get_height()
            new_width = 675
            # Calculate height to maintain aspect ratio
            new_height = int((original_height * new_width) / original_width)
            self.controls_image = pygame.transform.smoothscale(self.controls_image, (new_width, new_height))
        except Exception as e:
    ...
    def add_ufo(self, ufo):
        ...
        # Track UFOs spawned this level for messages
        self.ufos_spawned_this_level += 1
        
        # Track UFO type counts
        ufo_type = getattr(ufo, 'personality', 'unknown')
        if ufo_type not in self.ufo_type_counts:
            self.ufo_type_counts[ufo_type] = 0
        self.ufo_type_counts[ufo_type] += 1
    ...
    def calculate_time_dilation(self, dt):
        ...
        # Initial scoreboard refresh if no cached scores (only after game is fully initialized)
        if (self.scoreboard_available and self.scoreboard and 
            hasattr(self, 'game_state') and self.game_state == "waiting"):
    ...
    def init_ship(self):
        ...
        self.ship.level = self.level  # Set the level for ability charging logic
    ...
    def add_score(self, points, event_type=""):
        ...
        # Check for multiplier threshold messages (only if no higher priority messages are showing)
        if not (self.show_250k_message or self.show_100k_message or self.show_25k_message or 
                self.show_spinning_trick or self.show_interstellar or self.show_god_mode or 
                self.show_ludicrous_speed or self.show_plaid):
            
            # Check multiplier thresholds (only show once per threshold)
            if multiplier >= 5.0 and self.last_multiplier_threshold < 5:
                self.show_mult_5x_message = True
                self.mult_5x_timer = 3.0
                self.last_multiplier_threshold = 5
            elif multiplier >= 4.0 and self.last_multiplier_threshold < 4:
                self.show_mult_4x_message = True
                self.mult_4x_timer = 3.0
                self.last_multiplier_threshold = 4
            elif multiplier >= 3.0 and self.last_multiplier_threshold < 3:
                self.show_mult_3x_message = True
                self.mult_3x_timer = 3.0
                self.last_multiplier_threshold = 3
    ...
    def check_score_milestones(self):
        ...
        # Every 250,000 points: give +1 maximum lives and add ship indicator (capped at 5)
        ...
        if current_250k_milestone > self.last_milestone_250k:
            ...
            if self.max_lives < 5:
                self.max_lives += 1  # Increase maximum lives (capped at 5)
                if self.lives < self.max_lives:
                    self.lives += 1  # Add one life if current lives are below new max
            # Clear lower priority messages and show 250k message
            self.show_25k_message = False
            self.show_100k_message = False
            self.show_250k_message = True
            self.message_250k_timer = 3.0  # Show for 3 seconds
            # Still recharge shields and abilities for 250k milestone
            self.recharge_all_shields()
            self.recharge_all_abilities()
        
        # Every 100,000 points: recharge all shields and both ability rings
        current_100k_milestone = self.score // 100000
        if current_100k_milestone > self.last_milestone_100k:
            self.last_milestone_100k = current_100k_milestone
            self.recharge_all_shields()
            self.recharge_all_abilities()
            # Clear lower priority message and show 100k message
            self.show_25k_message = False
            self.show_100k_message = True
            self.message_100k_timer = 3.0  # Show for 3 seconds
        
        # Every 25,000 points: recharge all shields to maximum
        current_25k_milestone = self.score // 25000
        if current_25k_milestone > self.last_milestone_25k:
            self.last_milestone_25k = current_25k_milestone
            self.recharge_all_shields()
            # Show 25k message
            self.show_25k_message = True
            self.message_25k_timer = 3.0  # Show for 3 seconds
    ...
    def spawn_asteroids(self, count=None):
        ...
        for size_config in spawn_config['guaranteed']:
            ...
            if random.random() < 0.05:
                ...
                # 0.1% chance to add show_circle tag
                if random.random() < 0.001:
                    asteroid.tags.append("show_circle")
    ...
    def handle_input(self, dt):
        ...
        # C key handling disabled during gameplay to prevent crashes
        # C key only works on title screen and game over screen
        if pygame.K_c in self.keys_pressed and not hasattr(self, '_c_key_handled'):
            self._c_key_handled = True
        elif pygame.K_c not in self.keys_pressed and hasattr(self, '_c_key_handled'):
            # Reset the flag when key is released
            try:
                delattr(self, '_c_key_handled')
            except:
                pass
        ...
        # Ability activation (Q, B, or SHIFT keys) - only for in-game, title screen handled in KEYDOWN
        if (pygame.K_q in self.keys_pressed or pygame.K_b in self.keys_pressed or pygame.K_LSHIFT in self.keys_pressed or pygame.K_RSHIFT in self.keys_pressed):
            if self.game_state == "playing":
                self.activate_ability()
        
        # E key for brake (rapid deceleration)
        if pygame.K_e in self.keys_pressed:
            if self.game_state == "playing":
                self.ship.rapid_decelerate(dt)
        
        # Debug mode toggle (0 key) - with cooldown
        if pygame.K_0 in self.keys_pressed and self.f1_cooldown_timer <= 0:
        ...
        if len(self.ufos) > 0:
            ...
            for ufo in self.ufos[:]:
                ...
                if ufo.active:
                    ...
                    ufos_removed += 1
                    # Track UFO killed by player (ability blast)
                    self.ufos_killed_this_level += 1
                    self.total_ufos_destroyed_this_level += 1
                    # Log UFO destruction
                    game_logger.log_ufo_destroyed("ability blast", self.level)
    ...
    def activate_ability(self):
        ...
        try:
            if self.ship.ability_charges > 0 and not self.ship.ability_used and not self.ability_breaking:
                ...
                # Store current counts before resetting (for level completion tracking)
                self.pre_ability_asteroids_destroyed = self.asteroids_destroyed_this_level
                self.pre_ability_ufos_killed = self.ufos_killed_this_level
                self.pre_ability_total_ufos_destroyed = self.total_ufos_destroyed_this_level
            ...
            # Error in activate_ability - silent error handling
            pass
    ...
    def check_collisions(self):
            ...
            for bullet in self.bullets[:]:
                ...
                for asteroid in self.asteroids[:]:
                    ...
                    if self.check_wrapped_collision(bullet.position, asteroid.get_hitbox_center(), bullet.radius, asteroid.radius):
                        ...
                        # Check if this was a circled asteroid and give rewards
                        if "show_circle" in asteroid.tags:
                            # 100% chance to fully recharge shields
                            self.ship.shields = self.ship.max_shield_hits
                            # Trigger shield recharge animation
                            self.ship.shield_recharge_timer = 1.0  # 1 second animation
                            
                            # 100% chance to charge ability rings (1-2 based on current charges)
                            if self.ship.ability_charges < self.ship.max_ability_charges:
                                # Charge 1 ring if at 0, charge 2 rings if at 1
                                rings_to_add = 1 if self.ship.ability_charges == 0 else 2
                                self.ship.ability_charges = min(self.ship.max_ability_charges, self.ship.ability_charges + rings_to_add)
                                # Trigger ability recharge animation
                                self.ship.ability_recharge_pulse_timer = 1.0  # 1 second animation
                            
                            # Add UI message
                            self.show_nice_shot_message = True
                            self.nice_shot_timer = 2.0  # Show for 2 seconds
                        ...
                        if not (hasattr(asteroid, 'is_ability_asteroid') and asteroid.is_ability_asteroid):
                            ...
                            # Check if this was the last active asteroid after splitting
                            remaining_asteroids = len([a for a in self.asteroids if a.active])
                            if remaining_asteroids == 0:
                                # Last asteroid destroyed - make player invulnerable until new level
                                self.ship.invulnerable = True
                                self.ship.invulnerable_time = 10.0  # Long duration to cover level transition
                            break
                        else:
                            # For ability asteroids that don't split, check if this was the last one
                            remaining_asteroids = len([a for a in self.asteroids if a.active])
                            if remaining_asteroids == 0:
                                # Last asteroid destroyed - make player invulnerable until new level
                                self.ship.invulnerable = True
                    ...
                    if self.check_wrapped_collision(bullet.position, ufo.get_hitbox_center(), bullet.radius, ufo.radius):
                        ...
                        if ufo.spinout_active:
                            ...
                            if hasattr(ufo, 'spinout_collision_delay_timer') and hasattr(ufo, 'spinout_collision_delay'):
                                if ufo.spinout_collision_delay_timer >= ufo.spinout_collision_delay:
                                    ...
                                    self.ufos_killed_this_level += 1
                                    self.total_ufos_destroyed_this_level += 1  # Track UFO killed by player
                                    self.add_score(500, "ufo shot")  # 500 points for destroying UFO with shot
                                    # Log UFO destruction
                                    game_logger.log_ufo_destroyed("player shot", self.level)
                                    break
                                else:
                        ...
                        if random.random() < spinout_chance:
                            ...
                            try:
                                ...
                                # Add dedicated explosion particles (30 electric blue, 10 firey)
                                
                                # 30 electric blue sparks
                                for _ in range(30):
                                    self.explosions.add_explosion(
                                        ufo.position.x,
                                        ufo.position.y,
                                ...
                                # Still deactivate the UFO even if effects fail
                                ufo.active = False
                                self.ufos_killed_this_level += 1
                                self.total_ufos_destroyed_this_level += 1
                                self.add_score(500, "ufo shot")
                                game_logger.log_ufo_destroyed("player shot", self.level)
                ...
                for asteroid in self.asteroids:
                    ...
                    if self.check_wrapped_collision(self.ship.position, asteroid.get_hitbox_center(), self.ship.radius, asteroid.radius):
                        ...
                        if self.ship.shield_hits > 0:
                            ...
                            # Check if this was the last active asteroid
                            remaining_asteroids = len([a for a in self.asteroids if a.active])
                            if remaining_asteroids == 0:
                                # Last asteroid destroyed - make player invulnerable until new level
                                self.ship.invulnerable = True
                            ...
                            # Player destroyed by asteroid - no shields left
                            ...
                            if self.lives <= 0:
                                # Log player death
                                # Player died - transitioning to death_delay state
                                ...
                                # Player still alive - respawning ship
                                try:
                                    self.trigger_screen_shake(7, 0.6)  # Level 7 shake for player death
                                    self.init_ship()
                                    self.game_state = "playing"  # Set game state to playing after respawn
                                    # Ship respawned successfully
                                except Exception as e:
                                    # Error during respawn - silent error handling
                                    pass
                        break
        ...
        for ufo in self.ufos[:]:
            ...
            for asteroid in self.asteroids[:]:
                ...
                if self.check_wrapped_collision(ufo.get_hitbox_center(), asteroid.get_hitbox_center(), ufo.radius, asteroid.radius):
                    ...
                    # Check if UFO has spawn immunity - if so, skip damage but still split asteroid
                    if ufo.spawn_immunity_active:
                        # During spawn immunity: asteroid still splits, but UFO takes no damage
                        # Skip spinout chance and UFO damage during spawn immunity
                        pass
                    else:
                        # Normal collision behavior - check for spinout chance (5% for UFO vs Asteroid collision)
                    ...
                    if ufo.spinout_active:
                        ...
                        # Don't add score here - score is added when UFO is actually destroyed
        ...
        for i, ufo1 in enumerate(self.ufos[:]):
            ...
            for j, ufo2 in enumerate(self.ufos[i+1:], i+1):  # Avoid checking same pair twice
                ...
                if self.check_wrapped_collision(ufo1.position, ufo2.position, ufo1.radius, ufo2.radius):
                    # Check if either UFO has spawn immunity - if so, ignore collision
                    if ufo1.spawn_immunity_active or ufo2.spawn_immunity_active:
                        continue  # Skip collision processing during spawn immunity
                    ...
                    # Additional chance for UFO-to-UFO spinout collision to trigger spinout in the other UFO
                    if ufo1.spinout_active and not ufo2.spinout_active and random.random() < 0.3:  # 30% chance
                        ufo2.trigger_spinout()
                    elif ufo2.spinout_active and not ufo1.spinout_active and random.random() < 0.3:  # 30% chance
                        ufo1.trigger_spinout()
        ...
        # UFO vs Boss (with screen wrapping) - Spinning UFOs explode after collision delay, non-spinning pass through
        for ufo in self.ufos[:]:
            if not ufo.active:
                continue
            for boss in self.bosses[:]:
                if not boss.active:
                    continue
                # Check collision between UFO and boss using boss's polygon collision method
                if boss.polygon_circle_collision_with_wrapping(ufo.get_hitbox_center().x, ufo.get_hitbox_center().y, ufo.radius, self.current_width, self.current_height):
                    if ufo.spinout_active:
                        # Spinning UFO - no damage to boss, but check if delay has passed for UFO explosion
                        try:
                            if hasattr(ufo, 'spinout_collision_delay_timer') and hasattr(ufo, 'spinout_collision_delay'):
                                if ufo.spinout_collision_delay_timer >= ufo.spinout_collision_delay:
                                    # UFO in spinout mode and delay has passed - UFO explodes
                                    ...
                                    # Add score for UFO destruction after spinout
                                    self.ufos_killed_this_level += 1
                                    self.add_score(250, "ufo spun out")
                                    game_logger.log_ufo_destroyed("spinout collision", self.level)
                        except Exception as e:
                            ufo.active = False
                            ...
                            self.ufos_killed_this_level += 1
                            self.add_score(250, "ufo collision")
                            game_logger.log_ufo_destroyed("spinout collision (exception)", self.level)
                    else:
                        # Non-spinning UFO passes through boss harmlessly (no collision handling)
                        pass
                    break
        
        # Ship vs UFO (with screen wrapping)
        if self.ship.active and not self.ship.invulnerable and not self.god_mode:
            # Flag to prevent multiple UFO collisions per frame
            ufo_collision_handled = False
            for ufo in self.ufos:
                if not ufo.active:
                    continue
                if self.check_wrapped_collision(self.ship.position, ufo.get_hitbox_center(), self.ship.radius, ufo.radius):
                    ...
                    # Check if we've already handled a UFO collision this frame
                    if ufo_collision_handled:
                        continue
                    
                    # Mark that we've handled a UFO collision this frame
                    ufo_collision_handled = True
                    
                    # Check if UFO is in spinout mode
                    if ufo.spinout_active:
                        # UFO in spinout mode - instant damage
                        # UFO in spinout mode - damage player instantly
                        if self.ship.shield_hits > 0:
                            # Shield absorbs hit
                            self.ship.shield_hits -= 1
                            # Shield absorbed UFO hit
                            ...
                            # Add shot hit particles for collision
                            self.explosions.add_shot_hit_particles(ufo.position.x, ufo.position.y)
                            
                            # Destroy the UFO after collision
                            ufo.active = False
                            
                            # Add explosion particles for UFO (30 electric blue, 10 firey)
                            # 30 electric blue sparks
                            for _ in range(30):
                                self.explosions.add_explosion(
                                    ufo.position.x,
                            ...
                            for _ in range(10):
                                self.explosions.add_explosion(
                                    ufo.position.x,
                                    ufo.position.y,
                                    (255, 100, 0),  # Firey orange
                                    "firey"
                                )
                            
                            break  # Exit collision loop after shield absorption
                        else:
                            # No shields - ship destroyed
                            # Player destroyed by UFO (spinout mode) - no shields left
                        ...
                        # Normal UFO collision (not in spinout mode)
                        # Player hit by normal UFO
                        if self.ship.shield_hits > 0:
                            # Shield absorbs hit
                            self.ship.shield_hits -= 1
                            # Shield absorbed UFO hit
                            # Log player hit by UFO (normal mode, shield absorbed)
                            game_logger.log_player_hit_ufo(self.ship.shield_hits, self.lives)
                            ...
                            # No shields - ship destroyed
                            # Player destroyed by normal UFO - no shields left
                            # Log player hit by UFO (normal mode, ship destroyed)
                            game_logger.log_player_hit_ufo(0, self.lives)
            ...
            for bullet in self.ufo_bullets[:]:
                ...
                if self.check_wrapped_collision(self.ship.position, bullet.position, self.ship.radius, bullet.radius):
                    ...
                    if self.ship.shield_hits > 0:
                        # Shield absorbs hit
                        self.ship.shield_hits -= 1
                        # Shield absorbed UFO bullet hit
                        ...
                        # No shield, ship destroyed
                        # Player destroyed by UFO bullet - no shields left
                ...
                if self.ship.active and not self.ship.invulnerable and not self.god_mode:
                    if self.check_wrapped_collision(bullet.position, self.ship.position, bullet.radius, self.ship.radius):
                        ...
                        if self.ship.shield_hits > 0:
                            # Shield absorbs hit
                            self.ship.shield_hits -= 1
                            # Shield absorbed boss bullet hit
                            ...
                            # No shields - ship destroyed
                            # Player destroyed by boss bullet - no shields left
    ...
    def update(self, dt):
        ...
        if self.game_state == "waiting":
            ...
            # Debug logging for title screen updates
            if hasattr(self, 'title_ship') and self.title_ship is not None:
                ship_pos = f"({self.title_ship.position.x:.1f}, {self.title_ship.position.y:.1f})"
                ship_vel = f"({self.title_ship.velocity.x:.1f}, {self.title_ship.velocity.y:.1f})"
            ...
            # Controls fade in after press space to start fades in (starts at 5 seconds, fades over 3 seconds)
            if self.title_start_timer < 5.0:
                self.controls_alpha = 0  # Hidden until 5 seconds
            elif self.title_start_timer < 8.0:  # Fade in over 3 seconds (5-8 seconds)
                fade_progress = (self.title_start_timer - 5.0) / 3.0  # 0 to 1 over 3 seconds
        ...
        if self.game_state == "death_delay":
            ...
            if self.death_delay_timer >= 2.0:
                ...
                # Hide scoreboard on game over
                self.show_scoreboard = False
                
                # Automatically trigger name input if score qualifies
                if self.score > 0:  # Any positive score qualifies for name input
                ...
                if self.scoreboard_available and self.scoreboard:
                    ...
                    pass  # Scoreboard available, loading scores in background
                else:
                ...
                if self.score > 0:  # Any positive score qualifies for name input
                    self.name_input_active = True
                    self.player_name_input = ""
                    pass  # Auto-triggered name input for score
                else:
                    pass  # Score too low for name input
        ...
        if self.game_state not in ["playing", "paused", "death_delay"]:
            # Game state not in valid states
            return
        ...
        if self.ship:
            ...
            current_multiplier = self.calculate_multiplier()
            self.ship.update(dilated_dt, self.current_width, self.current_height, 1.0, dt, current_multiplier, len(self.asteroids), self.level)
            ...
            if is_shooting:
                current_multiplier = self.calculate_multiplier()
                rof_peak_reached = self.ship.update_rate_of_fire(dilated_dt, is_shooting, current_multiplier)
                if rof_peak_reached:
        ...
        if self.show_25k_message and self.message_25k_timer > 0:
            self.message_25k_timer -= 1.0 / 60.0  # Use fixed 60 FPS timing
            if self.message_25k_timer <= 0:
                self.show_25k_message = False
                
        if self.show_100k_message and self.message_100k_timer > 0:
            self.message_100k_timer -= 1.0 / 60.0  # Use fixed 60 FPS timing
            if self.message_100k_timer <= 0:
                self.show_100k_message = False
        ...
        # Check for UFO count thresholds (only if no higher priority messages are showing)
        if not (self.show_250k_message or self.show_100k_message or self.show_25k_message or 
                self.show_spinning_trick or self.show_interstellar or self.show_god_mode or 
                self.show_ludicrous_speed or self.show_plaid):
            
            # Check UFO count thresholds (only show once per threshold per level)
            if self.ufos_spawned_this_level >= 90 and not self.ufo_90_shown:
                self.show_ufo_90_message = True
                self.ufo_90_timer = 3.0
                self.ufo_90_shown = True
            elif self.ufos_spawned_this_level >= 60 and not self.ufo_60_shown:
                self.show_ufo_60_message = True
                self.ufo_60_timer = 3.0
                self.ufo_60_shown = True
            elif self.ufos_spawned_this_level >= 30 and not self.ufo_30_shown:
                self.show_ufo_30_message = True
                self.ufo_30_timer = 3.0
                self.ufo_30_shown = True
            elif self.ufos_spawned_this_level >= 20 and not self.ufo_20_shown:
                self.show_ufo_20_message = True
                self.ufo_20_timer = 3.0
                self.ufo_20_shown = True
            elif self.ufos_spawned_this_level >= 10 and not self.ufo_10_shown:
                self.show_ufo_10_message = True
                self.ufo_10_timer = 3.0
                self.ufo_10_shown = True
            
            # Check for more than 30 of any single UFO type
            if not self.ufo_too_many_shown:
                for ufo_type, count in self.ufo_type_counts.items():
                    if count > 30:
                        self.show_ufo_too_many_message = True
                        self.ufo_too_many_timer = 3.0
                        self.ufo_too_many_shown = True
                        break
        
        # Update lowest priority message timers
        if self.show_ufo_10_message and self.ufo_10_timer > 0:
            self.ufo_10_timer -= 1.0 / 60.0
            if self.ufo_10_timer <= 0:
                self.show_ufo_10_message = False
                
        if self.show_ufo_20_message and self.ufo_20_timer > 0:
            self.ufo_20_timer -= 1.0 / 60.0
            if self.ufo_20_timer <= 0:
                self.show_ufo_20_message = False
                
        if self.show_ufo_30_message and self.ufo_30_timer > 0:
            self.ufo_30_timer -= 1.0 / 60.0
            if self.ufo_30_timer <= 0:
                self.show_ufo_30_message = False
                
        if self.show_ufo_60_message and self.ufo_60_timer > 0:
            self.ufo_60_timer -= 1.0 / 60.0
            if self.ufo_60_timer <= 0:
                self.show_ufo_60_message = False
                
        if self.show_ufo_90_message and self.ufo_90_timer > 0:
            self.ufo_90_timer -= 1.0 / 60.0
            if self.ufo_90_timer <= 0:
                self.show_ufo_90_message = False
                
        if self.show_ufo_too_many_message and self.ufo_too_many_timer > 0:
            self.ufo_too_many_timer -= 1.0 / 60.0
            if self.ufo_too_many_timer <= 0:
                self.show_ufo_too_many_message = False
                
        if self.show_mult_3x_message and self.mult_3x_timer > 0:
            self.mult_3x_timer -= 1.0 / 60.0
            if self.mult_3x_timer <= 0:
                self.show_mult_3x_message = False
                
        if self.show_mult_4x_message and self.mult_4x_timer > 0:
            self.mult_4x_timer -= 1.0 / 60.0
            if self.mult_4x_timer <= 0:
                self.show_mult_4x_message = False
                
        if self.show_mult_5x_message and self.mult_5x_timer > 0:
            self.mult_5x_timer -= 1.0 / 60.0
            if self.mult_5x_timer <= 0:
                self.show_mult_5x_message = False
                
        if self.show_nice_shot_message and self.nice_shot_timer > 0:
            self.nice_shot_timer -= 1.0 / 60.0
            if self.nice_shot_timer <= 0:
                self.show_nice_shot_message = False
    ...
    def advance_level(self):
        ...
        # Store previous level data for cleanup
        previous_level = self.level
        
        # Use pre-ability counts if ability was used, otherwise use current counts
        if hasattr(self, 'pre_ability_asteroids_destroyed') and hasattr(self, 'pre_ability_ufos_killed'):
            asteroids_destroyed = self.pre_ability_asteroids_destroyed + self.asteroids_destroyed_this_level
            ufos_destroyed = self.pre_ability_total_ufos_destroyed + self.total_ufos_destroyed_this_level
            pass  # Using pre-ability counts
            # Clear the stored counts
            try:
                delattr(self, 'pre_ability_asteroids_destroyed')
                delattr(self, 'pre_ability_ufos_killed')
                delattr(self, 'pre_ability_total_ufos_destroyed')
            except AttributeError:
                pass  # Attributes already deleted or don't exist
        else:
            asteroids_destroyed = self.asteroids_destroyed_this_level
            ufos_destroyed = self.total_ufos_destroyed_this_level  # Use total count for level summary
            pass  # Using current counts
            
        current_score = self.score
        current_multiplier = self.calculate_multiplier()
        
        # Clean up gamelog for the new level (surgical cleanup)
        try:
            game_logger.cleanup_gamelog_for_new_level(previous_level, asteroids_destroyed, ufos_destroyed, current_score, current_multiplier)
        except Exception as e:
            print(f"Warning: Gamelog cleanup failed: {e}")
            # Continue with level advance even if cleanup fails
        ...
        self.asteroids_destroyed_this_level = 0
        self.ufos_killed_this_level = 0
        self.total_ufos_destroyed_this_level = 0
        
        # Reset UFO count tracking for new level
        self.ufos_spawned_this_level = 0
        self.ufo_type_counts = {}
        
        # Reset UFO message shown flags for new level (so messages can show again on each level)
        self.ufo_10_shown = False
        self.ufo_20_shown = False
        self.ufo_30_shown = False
        self.ufo_60_shown = False
        ...
        # Reset multiplier threshold tracking
        self.last_multiplier_threshold = 0
        
        # Clear all lowest priority messages
        self.show_ufo_10_message = False
        self.show_ufo_20_message = False
        self.show_ufo_30_message = False
        self.show_ufo_60_message = False
        ...
        # Reset level clear pause timer and star explosion state
        self.level_clear_pause_timer = 0.0
        self.star_explosion_active = False
        self.star_explosion_timer = 0.0
        
        # Reset star field modes
        self.star_field.explosion_mode = False
        ...
        # Add 1 life on new level (respects milestone-based maximum)
        if self.lives < self.max_lives:
            self.lives += 1
    ...
    def return_to_title_screen(self, log_current_score=False):
        ...
        # Hide scoreboard when returning to title screen
        self.show_scoreboard = False
        
        # Reset scoreboard opened flag when returning to title screen
        self.scoreboard_ever_opened = False
        ...
        self.last_milestone_25k = 0
        self.last_milestone_100k = 0
    ...
    def check_title_ufo_collisions(self):
        ...
        if not hasattr(self.title_ship, 'position') or self.title_ship.position is None:
            return
            
        if not hasattr(self, 'ufos') or not self.ufos:
            return
            
        if not hasattr(self, 'explosions') or self.explosions is None:
            return
            
        # Get player asteroid radius (size 2 asteroid)
        player_radius = 20  # Approximate radius for size 2 asteroid
        
        # Create a copy of the UFOs list to avoid modification during iteration
        ufos_to_check = [ufo for ufo in self.ufos if ufo is not None and hasattr(ufo, 'active') and hasattr(ufo, 'position')]
        
        for ufo in ufos_to_check:
            try:
                # Additional safety checks for each UFO
                if not ufo.active:
                    continue
                    
                if not hasattr(ufo, 'position') or ufo.position is None:
                    continue
                    
                if not hasattr(ufo.position, 'x') or not hasattr(ufo.position, 'y'):
                    continue
                
                # Validate position values
                if not isinstance(ufo.position.x, (int, float)) or not isinstance(ufo.position.y, (int, float)):
                    continue
                
                if not isinstance(self.title_ship.position.x, (int, float)) or not isinstance(self.title_ship.position.y, (int, float)):
                    continue
                ...
                if distance_squared < 0:  # Shouldn't happen, but safety check
                    continue
                ...
                if distance < (player_radius + 26):
                    # UFO destroyed
                    ufo.active = False
                    if hasattr(self, 'ufos_destroyed'):
                    ...
                    if hasattr(self, 'trigger_screen_shake'):
                        try:
                            ...
                            pass  # Ignore screen shake errors
                    
                    # Add explosion particles (same as in-game UFO destruction)
                    if hasattr(self, 'explosions') and self.explosions:
                        try:
                            total_particles = int(40 * 1.5 * 1.5)  # 50% more particles (60 * 1.5 = 90)
                            
                            # 90% electric blue spinout sparks
                            blue_particles = int(total_particles * 0.90)
                            self.explosions.add_explosion(
                                ufo.position.x,
                                ufo.position.y,
                                num_particles=blue_particles,
                                color=(0, 150, 255),  # Electric blue
                                is_ufo=True  # Mark as UFO explosion for proper handling
                            )
                            
                            # 10% bright white spinout sparks
                            white_particles = int(total_particles * 0.10)
                            self.explosions.add_explosion(
                                ufo.position.x,
                                ufo.position.y,
                                num_particles=white_particles,
                                color=(255, 255, 255),  # Bright white
                                is_ufo=True  # Mark as UFO explosion for proper handling
                            )
                        except Exception:
                            pass  # Ignore explosion errors to prevent crash
                            
            except Exception as e:
                # Log error but continue processing other UFOs
                print(f"Error in UFO collision check: {e}")
                continue
    ...
    def create_title_ship(self):
        ...
        if self.title_ship is None:
            try:
                ...
                width = getattr(self, 'current_width', 800)
                height = getattr(self, 'current_height', 600)
                ...
                self.title_ship = Ship(width // 2, height // 2)
            except Exception as e:
                print(f"Error creating title ship: {e}")
                # Fallback to default position
                self.title_ship = Ship(400, 300)
    ...
    def update_title_ship(self, dt):
        ...
        # Disable movement when scoreboard is open on title screen OR if scoreboard has ever been opened
        if (hasattr(self, 'show_scoreboard') and self.show_scoreboard and self.game_state == "waiting") or \
           (hasattr(self, 'scoreboard_ever_opened') and self.scoreboard_ever_opened and self.game_state == "waiting"):
            # Only update ship physics without input handling
        ...
        active_keys = []
        if left_rotate_pressed: active_keys.append("LEFT")
        if right_rotate_pressed: active_keys.append("RIGHT") 
        if left_strafe_pressed: active_keys.append("A")
        if right_strafe_pressed: active_keys.append("D")
        if up_pressed: active_keys.append("UP/W")
        if down_pressed: active_keys.append("DOWN/S")
        
        if active_keys:
            pass
        
        # Debug keys_pressed state
        if hasattr(self, 'keys_pressed') and len(self.keys_pressed) > 0:
            pressed_key_names = [pygame.key.name(key) for key in self.keys_pressed if key is not None]
        else:
            pass
        
        # Safety check: Remove TAB key from keys_pressed if scoreboard is closed
        if hasattr(self, 'show_scoreboard') and not self.show_scoreboard and hasattr(self, 'keys_pressed'):
            if pygame.K_TAB in self.keys_pressed:
                self.keys_pressed.discard(pygame.K_TAB)
        
        # Debug logging for scoreboard loading state during movement
        if hasattr(self, 'scoreboard_loading'):
            pass
        if hasattr(self, 'scoreboard_scores'):
            pass
        if hasattr(self, '_scoreboard_lock'):
            pass
    ...
    def draw(self, dt=0.016):
        ...
        if self.game_state == "playing":
            ...
            # Draw green hitbox circles for size 1 and 2 asteroids with show_circle tag (player layer)
            for asteroid in self.asteroids:
                if "show_circle" in asteroid.tags and asteroid.active and (asteroid.size == 1 or asteroid.size == 2):
                    original_x = asteroid.position.x
                    original_y = asteroid.position.y
                    asteroid.position.x += shake_x
                    asteroid.position.y += shake_y
                    
                    # Get hitbox center position
                    hitbox_center = asteroid.get_hitbox_center()
                    # Draw green circle for hitbox
                    pygame.draw.circle(draw_surface, (0, 255, 0), (int(hitbox_center.x), int(hitbox_center.y)), asteroid.radius, 2)
        ...
        if self.game_state == "playing":
            ...
            # Score milestone messages with priority (250k > 100k > 25k)
            if self.show_250k_message:
                message_250k_font = pygame.font.Font(None, 36)
                message_250k_text = message_250k_font.render("250k Extra Life + Fully Charged!", True, YELLOW)
                # Apply 15-degree skew effect
                ...
                message_y += 30
            elif self.show_100k_message:
                message_100k_font = pygame.font.Font(None, 36)
                message_100k_text = message_100k_font.render("100k Blast Double Charged!", True, YELLOW)
                # Apply 15-degree skew effect
                skewed_100k_text = self.create_skewed_message_text(message_100k_text, skew_factor=0.15)
                message_100k_rect = skewed_100k_text.get_rect(center=(self.current_width//2, message_y))
                draw_surface.blit(skewed_100k_text, message_100k_rect)
                message_y += 30
            elif self.show_25k_message:
                message_25k_font = pygame.font.Font(None, 36)
                message_25k_text = message_25k_font.render("25k Shields Recharged!", True, YELLOW)
                # Apply 15-degree skew effect
                skewed_25k_text = self.create_skewed_message_text(message_25k_text, skew_factor=0.15)
                message_25k_rect = skewed_25k_text.get_rect(center=(self.current_width//2, message_y))
                draw_surface.blit(skewed_25k_text, message_25k_rect)
                message_y += 30
            if self.show_nice_shot_message:
                nice_shot_font = pygame.font.Font(None, 36)
                nice_shot_text = nice_shot_font.render("Nice shot, kid! Shields are up!", True, (0, 255, 0))  # Green color
                skewed_nice_shot_text = self.create_skewed_message_text(nice_shot_text, skew_factor=0.15)
                nice_shot_rect = skewed_nice_shot_text.get_rect(center=(self.current_width//2, message_y))
                draw_surface.blit(skewed_nice_shot_text, nice_shot_rect)
                message_y += 30
            
            # Lowest priority UI messages (UFO count and multiplier) - only show if no higher priority messages
            elif self.show_ufo_90_message:
                ufo_90_font = pygame.font.Font(None, 32)
                ufo_90_text = ufo_90_font.render("90+ We've got incoming!", True, YELLOW)
                skewed_ufo_90_text = self.create_skewed_message_text(ufo_90_text, skew_factor=0.15)
                ufo_90_rect = skewed_ufo_90_text.get_rect(center=(self.current_width//2, message_y))
                draw_surface.blit(skewed_ufo_90_text, ufo_90_rect)
                message_y += 30
            elif self.show_ufo_60_message:
                ufo_60_font = pygame.font.Font(None, 32)
                ufo_60_text = ufo_60_font.render("60+ This is where the fun begins.", True, YELLOW)
                skewed_ufo_60_text = self.create_skewed_message_text(ufo_60_text, skew_factor=0.15)
                ufo_60_rect = skewed_ufo_60_text.get_rect(center=(self.current_width//2, message_y))
                draw_surface.blit(skewed_ufo_60_text, ufo_60_rect)
                message_y += 30
            elif self.show_ufo_30_message:
                ufo_30_font = pygame.font.Font(None, 32)
                ufo_30_text = ufo_30_font.render("30+ Stay on target… stay on target…", True, YELLOW)
                skewed_ufo_30_text = self.create_skewed_message_text(ufo_30_text, skew_factor=0.15)
                ufo_30_rect = skewed_ufo_30_text.get_rect(center=(self.current_width//2, message_y))
                draw_surface.blit(skewed_ufo_30_text, ufo_30_rect)
                message_y += 30
            elif self.show_ufo_20_message:
                ufo_20_font = pygame.font.Font(None, 32)
                ufo_20_text = ufo_20_font.render("Launch fighters!", True, YELLOW)
                skewed_ufo_20_text = self.create_skewed_message_text(ufo_20_text, skew_factor=0.15)
                ufo_20_rect = skewed_ufo_20_text.get_rect(center=(self.current_width//2, message_y))
                draw_surface.blit(skewed_ufo_20_text, ufo_20_rect)
                message_y += 30
            elif self.show_ufo_10_message:
                ufo_10_font = pygame.font.Font(None, 32)
                ufo_10_text = ufo_10_font.render("All ships, fire at will!", True, YELLOW)
                skewed_ufo_10_text = self.create_skewed_message_text(ufo_10_text, skew_factor=0.15)
                ufo_10_rect = skewed_ufo_10_text.get_rect(center=(self.current_width//2, message_y))
                draw_surface.blit(skewed_ufo_10_text, ufo_10_rect)
                message_y += 30
            elif self.show_ufo_too_many_message:
                ufo_too_many_font = pygame.font.Font(None, 32)
                ufo_too_many_text = ufo_too_many_font.render("There's too many of them!", True, YELLOW)
                skewed_ufo_too_many_text = self.create_skewed_message_text(ufo_too_many_text, skew_factor=0.15)
                ufo_too_many_rect = skewed_ufo_too_many_text.get_rect(center=(self.current_width//2, message_y))
                draw_surface.blit(skewed_ufo_too_many_text, ufo_too_many_rect)
                message_y += 30
            elif self.show_mult_5x_message:
                mult_5x_font = pygame.font.Font(None, 32)
                mult_5x_text = mult_5x_font.render("5x Great, kid, don't get cocky.", True, YELLOW)
                skewed_mult_5x_text = self.create_skewed_message_text(mult_5x_text, skew_factor=0.15)
                mult_5x_rect = skewed_mult_5x_text.get_rect(center=(self.current_width//2, message_y))
                draw_surface.blit(skewed_mult_5x_text, mult_5x_rect)
                message_y += 30
            elif self.show_mult_4x_message:
                mult_4x_font = pygame.font.Font(None, 32)
                mult_4x_text = mult_4x_font.render("4x The Force will be with you, always.", True, YELLOW)
                skewed_mult_4x_text = self.create_skewed_message_text(mult_4x_text, skew_factor=0.15)
                mult_4x_rect = skewed_mult_4x_text.get_rect(center=(self.current_width//2, message_y))
                draw_surface.blit(skewed_mult_4x_text, mult_4x_rect)
                message_y += 30
            elif self.show_mult_3x_message:
                mult_3x_font = pygame.font.Font(None, 32)
                mult_3x_text = mult_3x_font.render("3x We've got them on the run!", True, YELLOW)
                skewed_mult_3x_text = self.create_skewed_message_text(mult_3x_text, skew_factor=0.15)
                mult_3x_rect = skewed_mult_3x_text.get_rect(center=(self.current_width//2, message_y))
                draw_surface.blit(skewed_mult_3x_text, mult_3x_rect)
                message_y += 30
        ...
        if self.game_state == "waiting":
            ...
            # Show different message when scoreboard is open
            if hasattr(self, 'show_scoreboard') and self.show_scoreboard:
                subtitle_text = subtitle_font.render("PRESS TAB TO CLOSE SCOREBOARD", True, YELLOW)
            else:
            ...
            # Get local top score with level
            local_top_score, local_top_level = game_logger.get_top_local_score_with_level()
            local_score_display = f"{local_top_score:,} (Lv.{local_top_level})" if local_top_score is not None else "-"
            ...
            if global_top_score is not None:
                ...
                top_score_text = top_score_font.render(f"TOP SCORES:  Worldwide #1: {global_top_score:,}  -  Local #1: {local_score_display}", True, (255, 215, 0))  # Gold color
            else:
            ...
            top_score_rect = top_score_text.get_rect(center=(self.current_width//2, self.current_height//2 + 230))
            ...
            # Controls - display custom image with fade-in effect (moved up 60px)
            if self.controls_image is not None:
                # Get image dimensions and center it (moved up 60px from 200 to 140)
                controls_rect = self.controls_image.get_rect(center=(self.current_width//2 - 10, self.current_height//2 + 145))
                
                # Apply fade-in effect to controls image
                if self.controls_alpha < 255:
                    controls_surface = self.controls_image.copy()
                    controls_surface.set_alpha(int(self.controls_alpha))
                    draw_surface.blit(controls_surface, controls_rect)
                else:
                    draw_surface.blit(self.controls_image, controls_rect)
            
            # Second line of controls text (always displayed)
            controls_font = pygame.font.Font(None, 22)
            controls_text2 = controls_font.render("SCORES - tab  .  RESTART - r  .  PAUSE - p  .  EXIT - esc", True, (200, 200, 200))
            controls_rect2 = controls_text2.get_rect(center=(self.current_width//2, self.current_height//2 + 260))
            
            # Apply fade-in effect to second line
            if self.controls_alpha < 255:
            ...
            if self.show_scoreboard:
                try:
                    ...
                    # Disable scoreboard to prevent further crashes
                    self.show_scoreboard = False
            ...
            if self.show_scoreboard:
                ...
                inst_text = inst_font.render("C = Refresh", True, (200, 200, 200))
            ...
            if self.name_input_active:
                # Don't show additional instructions when name input is active - 
                # the draw_name_input function handles its own instructions
                pass
            else:
    ...
    def restart_game(self, log_current_score=False):
        ...
        # Reset scoreboard state to prevent rendering conflicts during restart
        ...
        # Reset name input state to prevent score submission issues
        self.name_input_active = False
        self.player_name_input = ""
        ...
        # Reset UFO message shown flags for new game (only reset on new game, not new level)
        self.ufo_10_shown = False
        self.ufo_20_shown = False
        self.ufo_30_shown = False
        self.ufo_60_shown = False
    ...
    def on_score_submitted(self, success, message):
        ...
        try:
            if success:
                # Mark submission as completed
                self.score_submission_completed = True
                # Refresh scoreboard in background
                self.load_scoreboard_background()
                print("Score submitted successfully!")
            else:
                # Check if it's a low score error and show user-friendly message
                if "Must be a number greater than 1000" in str(message):
                    print("Note: Scores must be above 1000 points to be submitted to the leaderboard.")
                    print("Your score will still be saved locally in the game log.")
                elif "already submitted" in str(message).lower():
                    print("Score already submitted for this game.")
                elif "already in progress" in str(message).lower():
                    print("Score submission already in progress, please wait...")
                elif "maximum submission attempts exceeded" in str(message).lower():
                    print("Score submission failed after multiple attempts. Please try again later.")
                else:
    ...
    def on_scores_loaded(self, scores):
        ...
        try:
            ...
            # Use lock to safely update the loading flag
            if hasattr(self, '_scoreboard_lock'):
    ...
    def load_scoreboard_background(self):
        ...
        try:
            ...
            # Don't load scores during game transitions or restart (only when actually playing)
            if (hasattr(self, 'game_start_timer') and 
                self.game_state == "playing" and 
                self.game_start_timer < 1.0):
                return
        except Exception as e:
            return
        
        # Use a lock to prevent race conditions
        if not hasattr(self, '_scoreboard_lock'):
            import threading
        ...
        with self._scoreboard_lock:
            if not self.scoreboard_loading:
                ...
                self.scoreboard_load_start_time = time.time()  # Record start time
                try:
    ...
    def spawn_bosses_for_level(self):
        ...
        if self.level == 3:
            ...
            # Level 12+: Variable number based on probability, with specific guarantees for certain levels
            if self.level == 12:
                # Level 12: Always 4 corner bosses
                self.spawn_four_corner_bosses()
            elif self.level == 21:
                # Level 21: Always 6 bosses (4 corners + 2 random)
                self.spawn_four_corner_bosses()
                self.spawn_multiple_bosses_random_sides(2)
            else:
                # Other levels 12+: Variable number based on probability
    ...
    def run(self):
        ...
        try:
            while self.running:
                ...
                try:
                    for event in pygame.event.get():
                        if event.type == pygame.QUIT:
                            ...
                            if self.name_input_active:
                                # Handle all key input when name input is active
                                # When name input is active, handle all keys for input and ignore all other key handling
                                try:
                                    if event.key == pygame.K_RETURN:
                                        # Submit the score
                                        try:
                                            # Check if submission is already in progress or completed
                                            if self.score_submission_in_progress:
                                                print("Score submission already in progress, please wait...")
                                            elif self.score_submission_completed:
                                                print("Score already submitted for this game.")
                                            elif self.player_name_input and self.player_name_input.strip():
                                            ...
                                            # Only close name input if submission is not in progress
                                            if not self.score_submission_in_progress:
                                                self.name_input_active = False
                                                self.player_name_input = ""
                                        ...
                                        # Safe backspace handling - only if not submitting
                                        if not self.score_submission_in_progress:
                                            try:
                                        ...
                                        # Limit total input length and handle special characters safely - only if not submitting
                                        if not self.score_submission_in_progress:
                                            try:
                                    ...
                                    self.name_input_active = False
                                    self.player_name_input = ""
                                # Skip all other key handling when name input is active
                                continue
                            
                            # All other key handling below - only processed when name input is NOT active
                            if event.key == pygame.K_SPACE and self.game_state == "waiting":
                                # Clean up title screen elements
                                self.cleanup_title_screen()
                                
                                # Hide scoreboard when starting new game
                                self.show_scoreboard = False
                                
                                # Reset scoreboard opened flag for new game
                                self.scoreboard_ever_opened = False
                                ...
                                try:
                                    ...
                                    print(f"Warning: Could not clear gamelog on game start: {e}")
                                    # Continue with game start even if cleanup fails
                                ...
                                self.star_explosion_active = True
                                self.star_field.start_explosion(self.current_width, self.current_height)
                            elif (event.key == pygame.K_q or event.key == pygame.K_b or event.key == pygame.K_LSHIFT or event.key == pygame.K_RSHIFT) and self.game_state == "waiting":
                                # Generate ability particles on title screen
                                self.generate_title_ability_particles()
                            elif self.game_state == "waiting":
                                # Block all keys except ESC, TAB, and C when scoreboard is open
                                if hasattr(self, 'show_scoreboard') and self.show_scoreboard:
                                    if event.key not in [pygame.K_ESCAPE, pygame.K_TAB, pygame.K_c]:
                                ...
                                if event.key == pygame.K_TAB:
                                    ...
                                    if self.scoreboard_available and self.scoreboard:
                                        old_state = self.show_scoreboard
                                        self.show_scoreboard = not self.show_scoreboard
                                        if self.show_scoreboard:
                                            # Mark that scoreboard has been opened on title screen
                                            self.scoreboard_ever_opened = True
                                            # Stop ship movement completely
                                            if self.title_ship:
                                                self.title_ship.velocity.x = 0
                                                self.title_ship.velocity.y = 0
                                            # Clear any held keys to prevent crashes
                                            if hasattr(self, 'keys_pressed'):
                                                keys_before = len(self.keys_pressed)
                                                self.keys_pressed.clear()
                                            # Load scores in background
                                            self.load_scoreboard_background()
                                        else:
                                            # Clear TAB key from keys_pressed to prevent movement conflicts
                                            if hasattr(self, 'keys_pressed'):
                                                self.keys_pressed.discard(pygame.K_TAB)
                                    else:
                                    ...
                                    if self.scoreboard_available and self.scoreboard:
                                        try:
                                            self.scoreboard.force_refresh_cache()
                                            # Always load scores in background when C is pressed
                                            self.load_scoreboard_background()
                                        except Exception as e:
                                            pass
                                    else:
                                    ...
                                    # ESC key quits the program from title screen
                                    self.running = False
                                ...
                                try:
                                    ...
                                    # Fallback: just set game state to waiting
                                    self.game_state = "waiting"
                                    if hasattr(self, 'show_scoreboard'):
                                        self.show_scoreboard = False
                                    if hasattr(self, 'keys_pressed'):
                                    ...
                                    if event.key == pygame.K_TAB:
                                        ...
                                        if self.scoreboard_available and self.scoreboard:
                                            ...
                                            if self.show_scoreboard:
                                                ...
                                                # Closing scoreboard on game over
                                                # Clear TAB key from keys_pressed to prevent movement conflicts
                                                if hasattr(self, 'keys_pressed'):
                                                    self.keys_pressed.discard(pygame.K_TAB)
                                        else:
                                        ...
                                        # Refresh scoreboard cache on game over screen
                                        if self.scoreboard_available and self.scoreboard:
                                            try:
                                                self.scoreboard.force_refresh_cache()
                                                if self.show_scoreboard:
                                ...
                                self.game_state = "playing"
                            elif event.key == pygame.K_ESCAPE and (self.game_state == "playing" or self.game_state == "paused"):
                                # ESC key quits the program from playing or paused state
                                # Log current score before exiting
                                if hasattr(self, 'score') and self.score > 0:
                                    game_logger.log_game_over(self.score, self.level)
                                self.running = False
                                ...
                                excluded_keys = {
                                    ...
                                    pygame.K_q, pygame.K_b, pygame.K_LSHIFT, pygame.K_RSHIFT,  # Q, B, SHIFT (abilities)
                                    pygame.K_e,  # E (brake)
                                ...
                                if event.key not in excluded_keys:
                                    ...
                                    # ESC key behavior - close score submission if open, otherwise quit
                                    if self.name_input_active:
                                        # Close score submission and return to normal game over screen
                                        self.name_input_active = False
                                        self.player_name_input = ""
                                    else:
                                        # Quit the program
                ...
                if self.score_submission_in_progress and self.submission_start_time > 0:
                    elapsed_time = time.time() - self.submission_start_time
                    if elapsed_time > self.submission_timeout:
                        print("Score submission timed out, resetting state...")
            ...
            print("c h u c k S T A R o i d s  1.o  ~  September 7 - 14, 2025  ~  x o x")
    ...
    def draw_scoreboard(self, surface):
        """Draw the scoreboard"""
        try:
            # Validate surface and dimensions before proceeding
            if not surface or not hasattr(surface, 'blit'):
                return
            ...
            # Additional safety check - ensure we're in a valid game state
            if not hasattr(self, 'game_state') or self.game_state not in ['playing', 'game_over', 'waiting']:
                return
            
            # Don't draw scoreboard during game transitions or when restarting
            # Only block if we're actually in a playing state and the timer is active
            if (hasattr(self, 'game_start_timer') and self.game_state == "playing" and 
                self.game_start_timer < 1.0):
                return
            ...
            try:
                ...
                # print(f"[SCOREBOARD DEBUG] Error creating overlay: {e}")
                # Don't return, just continue without overlay
                pass
            ...
            try:
                ...
                if not self.scoreboard_available or not self.scoreboard:
                    ...
                    title_text = font_large.render("WORLDWIDE TOP 10", True, WHITE)
                title_rect = title_text.get_rect(center=(self.current_width // 2, 100))
                surface.blit(title_text, title_rect)
            except Exception as e:
                # print(f"[SCOREBOARD DEBUG] Error rendering title: {e}")
                # Don't return, just continue with fallback
                try:
                    fallback_font = pygame.font.Font(None, 36)
                    fallback_title = fallback_font.render("SCOREBOARD", True, WHITE)
                    fallback_rect = fallback_title.get_rect(center=(self.current_width // 2, 100))
                    surface.blit(fallback_title, fallback_rect)
                except:
            ...
            try:
                ...
                try:
                    if not self.scoreboard_available or not self.scoreboard:
                        ...
                        # Check if loading has timed out (10 seconds)
                        if hasattr(self, 'scoreboard_load_start_time') and time.time() - self.scoreboard_load_start_time > 10.0:
                            formatted_scores = ["Loading timed out", "Press C to refresh"]
                            self.scoreboard_loading = False  # Reset loading state
                        else:
                            formatted_scores = ["Loading scores..."]
                    else:
                        # Ensure scoreboard_scores is valid before formatting
                        if not hasattr(self, 'scoreboard_scores') or self.scoreboard_scores is None:
                            self.scoreboard_scores = []
                        
                        # Check if we have valid scores
                        if not self.scoreboard_scores or len(self.scoreboard_scores) == 0:
                            formatted_scores = ["No scores available", "Press C to refresh"]
                        else:
                            formatted_scores = self.scoreboard.format_scores_display(self.scoreboard_scores)
                            
                        # Ensure we have at least one line to display
                        if not formatted_scores or len(formatted_scores) == 0:
                            formatted_scores = ["No scores available", "Press C to refresh"]
                            
                except Exception as e:
                    # print(f"[SCOREBOARD DEBUG] Error formatting scores: {e}")
                    formatted_scores = ["Error loading scores", "Press C to refresh"]
                
                # Draw each score line with additional safety checks
                for i, score_line in enumerate(formatted_scores):
                    try:
                        if not score_line or not isinstance(score_line, str):
                            # Draw a placeholder for empty lines
                        ...
                        # Limit line length to prevent rendering issues (increased for level display)
                        if len(score_line) > 65:
                            score_line = score_line[:62] + "..."
                            
                        score_text = font.render(score_line, True, WHITE)
                        if score_text:
                            score_rect = score_text.get_rect(center=(self.current_width // 2, y_offset + i * 35))
                            # Ensure the rect is within bounds
                            if (score_rect.x >= 0 and score_rect.y >= 0 and 
                                score_rect.x + score_rect.width <= self.current_width and 
                                score_rect.y + score_rect.height <= self.current_height):
                                surface.blit(score_text, score_rect)
                    except Exception as e:
                        ...
                        try:
                            ...
                            pass  # Skip this line if even error rendering fails
                
            except Exception as e:
                # print(f"[SCOREBOARD DEBUG] Error in scores section: {e}")
                # Fallback - draw at least something
                try:
                    fallback_text = font.render("Scoreboard Error", True, WHITE)
                    if fallback_text:
                        fallback_rect = fallback_text.get_rect(center=(self.current_width // 2, y_offset))
                        surface.blit(fallback_text, fallback_rect)
                except:
                    pass
            
            # Instructions - show close message only on title screen
            try:
                font_small = pygame.font.Font(None, 24)
                if hasattr(self, 'game_state') and self.game_state == "waiting":
                    # On title screen, show close instruction
                    inst_text = font_small.render("Press TAB to close", True, YELLOW)
                    if inst_text:
                        inst_rect = inst_text.get_rect(center=(self.current_width // 2, y_offset + 400))
                        surface.blit(inst_text, inst_rect)
            except Exception as e:
                # print(f"[SCOREBOARD DEBUG] Error rendering instructions: {e}")
                pass
    ...
    def draw_name_input(self, surface):
        ...
        try:
            ...
            # Change dialog box color based on submission state
            if self.score_submission_in_progress:
                pygame.draw.rect(surface, YELLOW, (dialog_x, dialog_y, dialog_width, dialog_height))
                pygame.draw.rect(surface, BLACK, (dialog_x + 2, dialog_y + 2, dialog_width - 4, dialog_height - 4))
            elif self.score_submission_completed:
                pygame.draw.rect(surface, GREEN, (dialog_x, dialog_y, dialog_width, dialog_height))
                pygame.draw.rect(surface, BLACK, (dialog_x + 2, dialog_y + 2, dialog_width - 4, dialog_height - 4))
            else:
            ...
            try:
                ...
                # Show cursor only if not submitting
                cursor = "_" if not self.score_submission_in_progress else ""
                name_text = font.render(safe_input + cursor, True, WHITE)
            ...
            # Instructions - show different messages based on submission state
            font_small = pygame.font.Font(None, 24)
            
            if self.score_submission_in_progress:
                # Show submission in progress message with animated dots
                dots = "." * (int(time.time() * 3) % 4)  # Animate dots
                inst_text = font_small.render(f"Submitting score{dots}", True, YELLOW)
                inst_rect = inst_text.get_rect(center=(self.current_width // 2, dialog_y + 150))
                surface.blit(inst_text, inst_rect)
                
                # Show attempt count if available
                if self.score_submission_attempts > 1:
                    attempt_text = font_small.render(f"Attempt {self.score_submission_attempts}/{self.max_submission_attempts}", True, YELLOW)
                    attempt_rect = attempt_text.get_rect(center=(self.current_width // 2, dialog_y + 170))
                    surface.blit(attempt_text, attempt_rect)
            elif self.score_submission_completed:
                # Show completion message
                inst_text = font_small.render("Score submitted successfully!", True, GREEN)
                inst_rect = inst_text.get_rect(center=(self.current_width // 2, dialog_y + 150))
                surface.blit(inst_text, inst_rect)
            else:
