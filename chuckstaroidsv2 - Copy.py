    ...
    def update(self, dt, screen_width=None, screen_height=None, time_dilation_factor=1.0, raw_dt=None):
        ...
        if self.shield_hits < self.max_shield_hits:
            ...
            if self.shield_recharge_time >= self.shield_recharge_duration:
                ...
                # If now at full shields (3/3), start full shield sequence
                if self.shield_hits == self.max_shield_hits:
        ...
        if self.shield_full_flash_timer > 0:
            ...
            # Start hold timer when flash ends
            if self.shield_full_flash_timer <= 0:
        ...
        # Update ring pulse timers (GAME TIME AFFECTED - uses dilated time for charging rings)
        ...
        if self.shield_hits > 0:
            ...
            if pulse_intensity > 0:
                ...
                for i in range(self.shield_hits):
                    ...
                    if self.shield_hits < self.max_shield_hits and i == self.shield_hits - 1:
                    ...
                    width = max(1, int(3 * ring_intensity * shield_pulse))
...
class AdvancedUFO(GameObject):
    def __init__(self, x, y, ai_personality="aggressive"):
        ...
        # Load UFO image
        try:
            self.image = pygame.image.load(get_resource_path("tie.gif"))
            self.image = self.image.convert_alpha()
...
class Game:
    ...
    def handle_input(self, dt):
        ...
        if pygame.K_r in self.keys_pressed:
            if self.game_state == "playing" and self.lives > 0:
                ...
                # During game over, R key goes to title screen (same as any other key)
                self.return_to_title_screen()
            # R key does nothing in other states (waiting, paused, etc.)
            return
    ...
    def update(self, dt):
        ...
        if self.game_state == "game_over":
            ...
            # Update star field during explosion or fade-in
            if self.star_explosion_active or self.star_field.fade_in_mode:
    ...
    def draw_life_indicators(self, surface, start_y, dt):
        ...
        # Center the middle ship, with others to left and right (offset 17px right, moved 3px left)
        if self.lives == 1:
            start_x = self.current_width // 2 - ship_size // 2 + 17
        elif self.lives == 2:
            start_x = self.current_width // 2 - ship_size // 2 - spacing // 2 + 17
        else:
            # For 3+ lives, center the middle ship
            # Calculate total width and center the group
            total_width = (self.lives - 1) * spacing + ship_size
            start_x = (self.current_width - total_width) // 2 + 17
    ...
    def create_gradient_title_text(self, text, font):
        """Create title text with bright yellow outline and gradient fill (yellow bottom to black top)"""
        ...
        # Create gradient fill - fade to black 50% faster (black at 50% height)
        for y in range(height):
            # Calculate gradient progress (0.0 at top, 1.0 at bottom)
            # Scale progress so black is reached at 50% height instead of 0%
            progress = y / height
            scaled_progress = min(1.0, progress * 2.0)  # 50% faster fade to black
    ...
    def draw(self, dt=0.016):
        ...
        if self.ship:
            ...
            # Draw star field during explosion or fade-in even when ship is not active
        ...
        if self.game_state == "waiting":
            ...
            # Scale animation: 0 to 100% over 3 seconds
            scale_progress = min(1.0, self.title_start_timer / 3.0)  # 0 to 1 over 3 seconds
            scale_factor = scale_progress  # 0 to 1
            ...
            if not self.star_explosion_active:
                ...
                # Apply fade-in effect to game over text
                if self.game_over_alpha < 255:
                    ...
                    draw_surface.blit(game_over_surface, (self.current_width//2 - 100, self.current_height//2 - 50))
                    draw_surface.blit(restart_surface, (self.current_width//2 - 120, self.current_height//2))
                else:
                ...
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        if event.key == pygame.K_SPACE and self.game_state == "waiting":
                            ...
                            # Any key press from game over goes to title screen
                            self.return_to_title_screen()
