...
class BossEnemy(GameObject):
    def __init__(self, x, y, direction="right", screen_width=1000, screen_height=750, level=3):
        ...
        if level == 0:  # Title screen boss - 25% from top of screen
            self.center_y = int(screen_height * 0.25)  # 25% from top of screen
        ...
        if direction == "left":
            ...
            # For title screen bosses, set Y to 25% from top
            if level == 0:
                self.position.y = int(screen_height * 0.25)
...
class Game:
    ...
    def update(self, dt):
        ...
        if self.game_state == "waiting":
            ...
            # Spawn title screen bosses instantly for testing (one from each side)
            if len(self.bosses) == 0:
                # Boss from left side
                left_boss = BossEnemy(0, 0, "left", self.current_width, self.current_height, level=0)
                self.bosses.append(left_boss)
                
                # Boss from right side
                right_boss = BossEnemy(0, 0, "right", self.current_width, self.current_height, level=0)
                self.bosses.append(right_boss)
