...
class Ship(GameObject):
    ...
    def update(self, dt, screen_width=None, screen_height=None, time_dilation_factor=1.0, raw_dt=None):
        ...
        # Calculate turning speed (degrees per second)
        if dt > 0:
        ...
        # Store current velocity and angle for next frame
...
class Game:
    def __init__(self):
        ...
        self.last_milestone_100k = 0  # Last 100k milestone reached
        self.last_milestone_250k = 0  # Last 250k milestone reached  
        self.last_milestone_500k = 0  # Last 500k milestone reached
        ...
        # Track first UFOs spawned on level 1 for 100% spinout chance
        ...
        # Sine wave movement for title screen UFOs
    ...
    def add_ufo(self, ufo):
        ...
        # Track first UFOs on level 1 for 100% spinout chance
        if self.level == 1 and self.first_ufos_spawned_level_1 < 3:  # First 3 UFOs on level 1
            self.first_ufos_spawned_level_1 += 1
    ...
    def calculate_time_dilation(self, dt):
        ...
        # Calculate turning-based movement (0.01 speed unit per degree per second)
        turning_movement = self.ship.turning_speed * 0.01
        ...
        if target_dilation > self.time_dilation_factor:
            ...
            current_turning = self.ship.turning_speed * 0.01
    ...
    def check_score_milestones(self):
        ...
        # Every 100,000 points: recharge all shields to maximum
        ...
        # Every 250,000 points: recharge all shields and both ability rings
        ...
        # Every 500,000 points: give +1 maximum lives and add ship indicator
    ...
    def draw_score_with_pulse(self, surface, dt):
        ...
        if self.score_pulse_timer < self.score_pulse_duration:
            ...
            # Calculate size scaling (1x at 1000 points to 5x at 5000+ points)
            if self.score_change >= 1000:
                # Linear scaling from 1x at 1000 points to 5x at 5000+ points
                # Formula: 1.0 + ((points - 1000) / 1000) * 4.0
                size_scale = 1.0 + ((self.score_change - 1000) / 1000.0) * 4.0  # 1.0 to 5.0
                size_scale = min(size_scale, 5.0)  # Cap at 5x
            else:
                size_scale = 1.0
        else:
    ...
    def check_collisions(self):
        ...
        for bullet in self.bullets[:]:
            ...
            for ufo in self.ufos[:]:
                ...
                if self.check_wrapped_collision(bullet.position, ufo.position, bullet.radius, ufo.radius):
                    ...
                    # Check if UFO is already in spinout mode (immune to projectiles)
                    if ufo.spinout_active:
                        # UFO is immune to projectiles during spinout, but still create hit particles
                    ...
                    # First 3 UFOs on level 1 have 100% chance to spinout
                    if self.level == 1 and self.first_ufos_spawned_level_1 < 3:
                        spinout_chance = 1.0
        ...
        for ufo in self.ufos[:]:
            ...
            for asteroid in self.asteroids[:]:
                if not asteroid.active:
                    continue
                if self.check_wrapped_collision(ufo.position, asteroid.get_hitbox_center(), ufo.radius, asteroid.radius):
        ...
        for i, ufo1 in enumerate(self.ufos[:]):
            ...
            for j, ufo2 in enumerate(self.ufos[i+1:], i+1):  # Avoid checking same pair twice
                ...
                if self.check_wrapped_collision(ufo1.position, ufo2.position, ufo1.radius, ufo2.radius):
                    # UFO collision - 5% chance for either to spinout
                    if not ufo1.spinout_active and not ufo2.spinout_active and random.random() < 0.05:  # 5% chance
                    ...
                    if ufo1.spinout_active and ufo1.spinout_collision_delay_timer >= ufo1.spinout_collision_delay:
                        # UFO1 in spinout and delay passed - destroy it
                        ufo1.active = False
                        ...
                        self.explosions.add_explosion(ufo1.position.x, ufo1.position.y, 
                                                    ...
                                                    color=(255, 255, 255), is_ufo=True)
                        self.add_score(200)
                    elif not ufo1.spinout_active:
                        # UFO1 not in spinout - normal collision damage
                        ufo1.active = False
                        self.explosions.add_explosion(ufo1.position.x, ufo1.position.y, 
                                                    num_particles=45, 
                                                    color=(0, 150, 255), is_ufo=True)
                    ...
                    if ufo2.spinout_active and ufo2.spinout_collision_delay_timer >= ufo2.spinout_collision_delay:
                        # UFO2 in spinout and delay passed - destroy it
                        ufo2.active = False
                        ...
                        self.explosions.add_explosion(ufo2.position.x, ufo2.position.y, 
                                                    ...
                                                    color=(255, 255, 255), is_ufo=True)
                        self.add_score(200)
                    elif not ufo2.spinout_active:
                        # UFO2 not in spinout - normal collision damage
                        ufo2.active = False
                        self.explosions.add_explosion(ufo2.position.x, ufo2.position.y, 
                                                    num_particles=45, 
                                                    color=(0, 150, 255), is_ufo=True)
        ...
        if self.ship.active and not self.ship.invulnerable and not self.god_mode:
            for ufo in self.ufos:
                ...
                if self.check_wrapped_collision(self.ship.position, ufo.position, self.ship.radius, ufo.radius):
                    ...
                    if ufo.spinout_active:
                        if ufo.spinout_collision_delay_timer >= ufo.spinout_collision_delay:
                            # UFO in spinout mode and delay has passed - normal collision damage
                            if self.ship.shield_hits > 0:
                                # Shield absorbs hit
                                self.ship.shield_hits -= 1
                                # Don't reset recharge timer - let charging continue
    ...
    def update(self, dt):
        ...
        if self.ship:
            ...
            self.ship.update(dilated_dt, self.current_width, self.current_height, 1.0, dt)
            
            # Track rotations for "spinning trick" achievement (only while actively spinning)
            if not self.ship.spinning_trick_shown and self.ship.is_spinning:
                # Calculate angle difference, handling wraparound
                ...
                self.ship.total_rotations += abs(angle_diff) / (2 * math.pi)
            ...
            elif not self.ship.is_spinning:
        ...
        if self.ship and self.ship.spinning_trick_shown and not self.show_spinning_trick:
            ...
            self.ludicrous_speed_timer = 0.0
            self.plaid_timer = 0.0
            # Show spinning trick message
            self.show_spinning_trick = True
            self.spinning_trick_timer = 3.0  # Show for 3 seconds
            # Reset the flag and rotation counter so it can trigger again
    ...
    def cleanup_title_screen(self):
        ...
        self.initial_ufo_timer = 5.0
        self.ufos_to_spawn = 0
        self.ufo_spawn_delay = 0
        self.ufo_spawn_corner = None
        self.ufo_mass_spawn = False
        self.ufo_spawn_types = None
    ...
    def update_title_ship(self, dt):
        ...
        # Update ship physics
        self.title_ship.update(dt, self.current_width, self.current_height)
    ...
    def draw(self, dt=0.016):
        ...
        if self.game_state == "playing":
            # Layer 1: Draw asteroids (bottom layer)
            for asteroid in self.asteroids:
            ...
            # Layer 2: Draw explosion particles (above asteroids)
            self.explosions.draw(draw_surface)
            
            # Layer 3: Draw UFOs (above particles)
            for ufo in self.ufos:
                ...
                ufo.draw(draw_surface, self.debug_mode, shake_x, shake_y)
                # Restore original position
                ufo.position.x = original_x
                ufo.position.y = original_y
            
            # Layer 4: Draw all bullets (above UFOs)
            for bullet in self.bullets:
            ...
            # Layer 5: Draw player ship (on top of everything)
            if self.ship:
        ...
        if self.game_state == "playing":
            ...
            # Speed display - below life indicators
        ...
        # Draw debug speed display if debug mode is enabled
        if self.debug_mode:
                ...
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        if event.key == pygame.K_SPACE and self.game_state == "waiting":
                            ...
                            print("Title effects are disabled")
