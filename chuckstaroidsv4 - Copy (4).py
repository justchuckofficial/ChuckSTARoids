...
class Game:
    ...
    def add_score(self, points, event_type=""):
        ...
        # Log detailed scoring event with new format
        import datetime
        current_time = datetime.datetime.now().strftime("%H:%M:%S.%f")[:-3]  # Include milliseconds
        
        if event_type:
            print(f"[{current_time}] {event_type}: +{points}, +{multiplier_gained:.3f}, x{multiplier:.3f}, ={self.score}")
        else:
            print(f"[{current_time}] SCORE EVENT: +{points} points, x{multiplier:.2f} multiplier = {multiplied_points} total, Score: {old_score} -> {self.score}, Current Multiplier: {multiplier:.2f}")
        ...
        for ufo in self.ufos[:]:
            if ufo.active:
                ...
                self.explosions.add_explosion(ufo.position.x, ufo.position.y, 
                                            num_particles=5, 
                                            color=(255, 255, 255), is_ufo=True, use_raw_time=True)
                
                # 45 electric blue particles
                self.explosions.add_explosion(ufo.position.x, ufo.position.y, 
                                            num_particles=45, 
                                            color=(0, 100, 255), is_ufo=True, use_raw_time=True)
            ...
            if total_objects > 0:
                ...
                for ufo in self.ufos[:]:
                    if ufo.active:
                        ...
                        self.explosions.add_explosion(ufo.position.x, ufo.position.y, 
                                                    num_particles=6, color=(255, 255, 255), 
                                                    asteroid_size=None, is_ufo=True, use_raw_time=True)
                        # 2 yellow particles
                        self.explosions.add_explosion(ufo.position.x, ufo.position.y, 
                                                    num_particles=2, color=(255, 255, 0), 
                                                    asteroid_size=None, is_ufo=True, use_raw_time=True)
    ...
    def draw_debug_fps_display(self, surface):
        ...
        # Collision FPS information
        collision_texts = []
        for collision_type, fps in self.current_collision_fps.items():
            if fps < 60:  # Only show non-60 FPS collision types
                collision_texts.append(f"{collision_type}: {fps:.0f}")
        ...
        # Draw collision FPS information
        if collision_texts:
            y_offset = text_rect.bottom + 5
            for i, collision_text in enumerate(collision_texts[:5]):  # Show max 5 collision types
                collision_surface = debug_font.render(collision_text, True, (255, 255, 0))
                collision_rect = collision_surface.get_rect()
                collision_rect.x = self.current_width - collision_rect.width - 10
                collision_rect.y = y_offset + (i * 22)
                
                # Draw semi-transparent background for collision text
                collision_bg_rect = collision_rect.copy()
                collision_bg_rect.width += 10
                collision_bg_rect.height += 2
                collision_bg_rect.x -= 5
                collision_bg_rect.y -= 1
                pygame.draw.rect(surface, (0, 0, 0, 128), collision_bg_rect)
                
                # Draw collision text
                surface.blit(collision_surface, (collision_rect.x, collision_rect.y))
